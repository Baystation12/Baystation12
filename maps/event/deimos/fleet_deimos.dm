...
/mob/living/simple_animal/hostile/hivebot/ranged_damage/deimos
	...
	ai_holder = /datum/ai_holder/simple_animal/hostile/hivebot/ranged_damage/deimos
...
/mob/living/simple_animal/hostile/hivebot/ranged_damage/deimos/death()
...
/mob/living/simple_animal/hostile/hivebot/ranged_damage/deimos/proc/flash(delay)
...
/mob/living/simple_animal/hostile/hivebot/ranged_damage/deimos/proc/detonate()
	// The actual boom.
	if (src && !exploded)
...
/mob/living/simple_animal/hostile/hivebot/ranged_damage/deimos/ion
...
/mob/living/simple_animal/hostile/hivebot/ranged_damage/deimos/ballistic
...
/mob/living/simple_animal/hostile/deimosheavy
	...
	ai_holder = /datum/ai_holder/simple_animal/ranged/kiting/threatening/deimosheavy
...
/mob/living/simple_animal/hostile/deimosheavy/ranged_pre_animation(atom/A)
	//Create an effect that will ease out
	//we could use do_attack_effect here, but doesnt give a lot of freedom for animation

	var/image/I = image('maps/event/deimos/smallrobot.dmi', A, "lockonbig")
...
/mob/living/simple_animal/hostile/deimosheavy/shoot_target(atom/A)
	set waitfor = FALSE
...
/mob/living/simple_animal/hostile/deimosheavy/ranged_post_animation(atom/A)
...
/mob/living/simple_animal/hostile/deimosheavy/death()
...
/mob/living/simple_animal/hostile/deimosheavy/proc/flash(delay)
...
/mob/living/simple_animal/hostile/deimosheavy/proc/detonate()
	// The actual boom.
	if (src && !exploded)
		src.visible_message(SPAN_DANGER("\The [src]'s body detonates!"))
		exploded = TRUE
...
/mob/living/simple_animal/hostile/deimosheavy/Allow_Spacemove(check_drift)
	return TRUE
...
/mob/living/simple_animal/hostile/deimosheavy/Initialize()
	. = ..()
	new /obj/aura/mobshield(src)

/obj/item/clothing/under/solgov/utility/deimosvacsuit
	...
	item_icons = list(slot_w_uniform_str = 'maps/event/deimos/smallrobot.dmi')
	icon = 'maps/event/deimos/smallrobot.dmi'
	icon_state = "deimosvacsuit"
	worn_state = "deimosvacsuit"
	armor = list(
...
/obj/structure/decorative/deimoscrate
	name = "armored container"
	desc = "A massive, densely plated container with countless safety latches and gauges. The central viewport glows with an eerie blue light."
	icon = 'maps/event/deimos/smallrobot.dmi'
	icon_state = "mcgoofy"
	anchored = FALSE
	density = TRUE
...
/obj/structure/decorative/deimoscrate/Initialize()
	. = ..()
	crate_sound = GLOB.sound_player.PlayLoopingSound(src, "\ref[src]", 'sound/machines/BSD_idle.ogg', 50, 7)
	overlays += overlay_image(icon, "mcgoofy_light", plane = EFFECTS_ABOVE_LIGHTING_PLANE, layer = ABOVE_LIGHTING_LAYER)
	set_light(1, 2, 4, 10)

/obj/structure/decorative/deimoscrate/Move()
	. = ..()
	if(.)
...
/obj/item/clothing/accessory/medal/fa
	...
	icon = 'maps/event/deimos/medals.dmi'
	accessory_icons = list(
		slot_w_uniform_str = 'maps/event/deimos/medals.dmi',
		slot_wear_suit_str = 'maps/event/deimos/medals.dmi'
	)
