...
/datum/map_template/ruin/antag_spawn/ert/after_load(Z)
	..(Z)
	if(Z)
		GLOB.using_map.admin_levels |= GetConnectedZlevels(Z)
		GLOB.using_map.escape_levels |= GetConnectedZlevels(Z)
	var/obj/machinery/computer/shuttle_control/multi/ert/E = locate() in SSmachines.machinery
	if(E)
		GLOB.ert_announcer.forceMove(get_turf(E))
...
/obj/machinery/rotating_alarm/start_on/ert/Initialize()
	. = ..()
	spawn(1 MINUTE)
		if(sound_loop)
