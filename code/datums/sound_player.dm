...
/singleton/sound_player/proc/PlaySoundDatum(atom/source, sound_id, sound/new_sound, range, prefer_mute)
	var/token_type = isnum(new_sound.environment) ? /datum/sound_token : /datum/sound_token/static_environment
	return new token_type(source, sound_id, new_sound, range, prefer_mute)

/singleton/sound_player/proc/PlayLoopingSound(atom/source, sound_id, _sound, volume, range, falloff = 1, echo, frequency, prefer_mute)
...
/datum/sound_token/New(atom/source, sound_id, sound/new_sound, range = 4, prefer_mute = FALSE)
	..()
	if(!istype(source))
