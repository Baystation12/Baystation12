...
/datum/proximity_trigger/proc/register_turfs()
	if(ismovable(holder))
		GLOB.moved_event.register(holder, src, /datum/proximity_trigger::on_holder_moved())
	GLOB.dir_set_event.register(holder, src, /datum/proximity_trigger::register_turfs()) // Changing direction might alter the relevant turfs
	...
	for(var/t in (turfs_in_range - new_turfs))
		GLOB.opacity_set_event.unregister(t, src, /datum/proximity_trigger::on_turf_visibility_changed())
	for(var/t in (new_turfs - turfs_in_range))
		GLOB.opacity_set_event.register(t, src, /datum/proximity_trigger::on_turf_visibility_changed())
...
/datum/proximity_trigger/proc/unregister_turfs()
	if(ismovable(holder))
		GLOB.moved_event.unregister(holder, src, /datum/proximity_trigger::on_holder_moved())
	GLOB.dir_set_event.unregister(holder, src, /datum/proximity_trigger::register_turfs())

	for(var/t in turfs_in_range)
		GLOB.opacity_set_event.unregister(t, src, /datum/proximity_trigger::on_turf_visibility_changed())
	for(var/t in seen_turfs_)
		GLOB.entered_event.unregister(t, src, /datum/proximity_trigger::on_turf_entered())
...
/datum/proximity_trigger/proc/on_turf_visibility_changed()
	...
	for(var/t in (new_seen_turfs_ - seen_turfs_))
		GLOB.entered_event.register(t, src, /datum/proximity_trigger::on_turf_entered())
...
/obj/item/proxy_debug/New()
	...
	var/datum/proximity_trigger/a = new proxy_type(src, /obj/item/proxy_debug::turf_entered(), /obj/item/proxy_debug::update_turfs())
	a.register_turfs()
