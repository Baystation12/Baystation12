...
/datum/disease/proc/stage_act()
	...
	if(stage > max_stages)
		stage = max_stages
	if(!cure_present && prob(stage_prob)) //now the disease shouldn't get back up to stage 4 in no time		stage = min(stage + 1, max_stages)	else if(cure_present && prob(cure_chance))		stage = max(stage - 1, 1)	if(stage <= 1 && ((prob(1) && curable) || (cure_present && prob(cure_chance))))		cure()
		return
	return
