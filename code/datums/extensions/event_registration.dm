...
/datum/extension/event_registration/New(datum/holder, singleton/observ/event, datum/target, callproc)
	..()
	event.register(target, src, src::trigger())
	GLOB.destroyed_event.register(target, src, src::qdel_self())
	src.event = event
	src.target = target
...
/datum/extension/event_registration/shuttle_stationary/New(datum/holder, singleton/observ/event, datum/target, callproc, area/given_area)
	...
	register_shuttles()
	GLOB.shuttle_added.register_global(src, src::shuttle_added())
	...
	if(given_area in SSshuttle.shuttle_areas)
		for(var/shuttle_name in SSshuttle.shuttles)
			...
			if(given_area in shuttle_datum.shuttle_area)
				GLOB.shuttle_moved_event.register(shuttle_datum, src, src::shuttle_moved())
				GLOB.shuttle_pre_move_event.register(shuttle_datum, src, src::shuttle_pre_move())
...
/datum/extension/event_registration/shuttle_stationary/proc/shuttle_added(datum/shuttle/shuttle)
	if(given_area in shuttle.shuttle_area)
		GLOB.shuttle_moved_event.register(shuttle, src, src::shuttle_moved())
		GLOB.shuttle_pre_move_event.register(shuttle, src, src::shuttle_pre_move())
