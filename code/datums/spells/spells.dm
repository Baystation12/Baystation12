...
#define SPELL_CHARGES 2
#define SPELL_HOLDERVAR 3

/*
Okay, so here's how spells work.
spell_list is a mob-level list var. To give a mob a spell, just add the spell holder to that list.
Spell's Click() calls cast_check(). If it returned true, choose_targets() is called.
cast_check() will use up charges if the spell can be used
choose_targets() collects a list of targets and calls perform() on them. If the list is empty, it calls revert_cast() instead.
perform() calls before_cast(targets), then invocation(), then, in a separate spawn(), start_recharge() if the spell is rechargeable, then cast(targets), then after_cast(targets)
before_cast(targets), by default, creates an overlay if needed
invocation() makes the user shout the invocation
start_recharge() recharges the spell
cast(targets) does nothing by default - this is where you do your magic
after_cast(targets) handles creation of smoke and sparks
*/
...
/obj/effect/proc_holder/spell
	...
	var/clothes_req = 1 //see if it requires clothes - this implies the need to be a human
...
/obj/effect/proc_holder/spell/proc/cast_check(skipcharge = 0, mob/user = usr) //checks if the spell can be cast based on its settings; skipcharge is used when an additional cast_check is called inside the spell
	...
	if(!(src in usr.spell_list))
		usr << "<span class='danger'>You shouldn't have this spell! Something's wrong.</span>"
		return 0
...
/obj/effect/proc_holder/spell/proc/perform(list/targets, recharge = 1) //if recharge is started is important for the trigger spells
	...
	invocation()
	spawn(0)
		if(charge_type == SPELL_RECHARGE && recharge)
			start_recharge()
	cast(targets)
	after_cast(targets)
