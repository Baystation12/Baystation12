...
/datum/beam/proc/Start()
	...
	if(istype(origin, /atom/movable))
		GLOB.moved_event.register(origin, src, src::redrawing())
	if(istype(target, /atom/movable))
		GLOB.moved_event.register(target, src, src::redrawing())
	GLOB.destroyed_event.register(origin, src, src::redrawing())
	GLOB.destroyed_event.register(target, src, src::redrawing())
...
/datum/beam/Destroy()
	QDEL_NULL_LIST(elements)
	QDEL_NULL(visuals)
	GLOB.moved_event.unregister(origin, src, src::redrawing())
	GLOB.moved_event.unregister(target, src, src::redrawing())
	GLOB.destroyed_event.unregister(origin, src, src::redrawing())
	GLOB.destroyed_event.unregister(target, src, src::redrawing())
	target = null
	origin = null
	return ..()
