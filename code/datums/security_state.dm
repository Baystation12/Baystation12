...
/singleton/security_level/default/proc/notify_station()
	for (var/obj/machinery/firealarm/FA as anything in MACHINES_OF(/obj/machinery/firealarm))
		if(FA.z in GLOB.using_map.contact_levels)
			FA.update_icon()
	for (var/obj/machinery/rotating_alarm/security_alarm/SA in MACHINES_OF(/obj/machinery/rotating_alarm/security_alarm))
