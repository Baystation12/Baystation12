...
/decl/security_state/New()
	...
	severe_security_level = Singletons.get_decl(severe_security_level)
	...
	high_security_level = Singletons.get_decl(high_security_level)
	...
	if(highest_standard_security_level || isnull(highest_standard_security_level))
		...
		highest_standard_security_level = Singletons.get_decl(highest_standard_security_level)
	else
		highest_standard_security_level = null

	// Setup the current security level
	if(current_security_level in all_security_levels)
		current_security_level = Singletons.get_decl(current_security_level)
	else
		current_security_level = Singletons.get_decl(all_security_levels[1])
	...
	for(var/security_level_type in all_security_levels)
		security_level_instances += Singletons.get_decl(security_level_type)
