...
//				text += "<br>All the changelings are dead! Restart in [round((changeling.TIME_TO_GET_REVIVED-(world.time-changeling.changelingdeathtime))/10)] seconds."
	...
	Topic(href, href_list)
		...
		if (href_list["role_edit"])
			...
			var/new_memo = copytext(sanitize_russian(input("Write new memory", "Memory", memory) as null|message),1,MAX_MESSAGE_LEN)
			...
			switch (new_obj_type)
				...
				if ("custom")
					var/expl = copytext(sanitize_russian(input("Custom objective:", "Objective", objective ? objective.explanation_text : "") as text|null),1,MAX_MESSAGE_LEN)
					if (!expl) return
