/proc/send_signal(var/datum/target, var/sigtype, ...)
	var/list/signal_args = list()
	if(length(args) > 2)
		signal_args = args.Copy(3)
	return ( !target.comp_lookup || !target.comp_lookup[sigtype] ? NONE : target._SendSignal(sigtype, signal_args) )
