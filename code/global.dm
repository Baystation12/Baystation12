...
var/datum/nanomanager/nanomanager = new()
	...
	//airlockWireColorToIndex takes a number representing the wire colour, e.g. the orange wire is always 1, the dark red wire is always 2, etc. It returns the index for whatever that wire does.
	//airlockIndexToWireColor does the opposite thing - it takes the index for what the wire does, for example AIRLOCK_WIRE_IDSCAN is 1, AIRLOCK_WIRE_POWER1 is 2, etc. It returns the wire colour number.
	//airlockWireColorToFlag takes the wire colour number and returns the flag for it (1, 2, 4, 8, 16, etc)
