...
//literally the worst place to put this ever - it needs it's own file, C
/obj/effect/datacore
	name = "datacore"
	var/medical[] = list()
	var/general[] = list()
...
/obj/item/device/infra_sensor
	...
	proc/get_manifest(monochrome)
		...
		<head><style>
			...
			.manifest th {height: 2em; [monochrome?"border-top-width: 3px":"background-color: #48C; color:white"]}
			.manifest tr.head th { [monochrome?"border-top-width: 1px":"background-color: #488;"] }
			.manifest td:first-child {text-align:right}			.manifest tr.alt td {[monochrome?"border-top-width: 2px":"background-color: #DEF"]}
		</style></head>
		<table class="manifest">
		<tr class='head'><th>Name</th><th>Rank</th></tr>
		"}
