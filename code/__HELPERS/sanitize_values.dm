//general stuff
/proc/sanitize_russian_integer(number, min=0, max=1, default=0)
	if(isnum(number))
		number = round(number)
...
/proc/sanitize_russian_text(text, default="")
	if(istext(text))
		return text
	return default

/proc/sanitize_russian_inlist(value, list/List, default)
...
/proc/sanitize_russian_gender(gender,neuter=0,plural=0, default="male")
	switch(gender)
...
/proc/sanitize_russian_hexcolor(color, default="#000000")
