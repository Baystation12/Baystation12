...
/mob/new_player/proc/new_player_panel(force)
	...
	if (config.wiki_url || config.rules_url || config.lore_url)
		...
		if (isnum(player_age) && player_age < 7)
			output += "<b>Bienvenido! Por favor, echa un vistazo a estos enlaces:</b><br>"
		if (config.wiki_url)
			output += "<a href='byond://?src=\ref[src];show_wiki=1'>Wiki</a>"
		if (config.rules_url)
			output += "<a href='byond://?src=\ref[src];show_rules=1'>Normas</a>"
		if (config.lore_url)
			output += "<a href='byond://?src=\ref[src];show_lore=1'>Lore</a>"
	output += "<hr>"
	if (GAME_STATE > RUNLEVEL_LOBBY)
		output += "<a href='byond://?src=\ref[src];manifest=1'>Manifesto</a>"
	output += "<a href='byond://?src=\ref[src];show_preferences=1'>Opciones</a>"
	output += "<hr>"
	output += "<b>El nombre de tu personaje es:</b><br>"
	...
	output += "<hr>"
	output += "<a href='byond://?src=\ref[src];observe=1'>Unete como observador</a>"
	if (GAME_STATE > RUNLEVEL_LOBBY)
		output += "<a href='byond://?src=\ref[src];late_join=1'>Unirse como tripulante</a>"
	else
		output += "<a [ready?"class='linkOn'":""] href='byond://?src=\ref[src];ready=[!ready]'>Unirse al inicio de la ronda</a>"
	output += "<hr>"
	output += "<i>[GLOB.using_map.get_map_info()||"No information available for the current map."]</i>"
	output += "</div>"
	panel = new (src, "Bienvenido","Bienvenido a la [GLOB.using_map.full_name]", 560, 340, src)
...
/mob/new_player/Stat()
	...
	if(statpanel("Lobby"))
		if(check_rights(R_INVESTIGATE, 0, src))
			stat("Modo de juego:", "[SSticker.mode ? SSticker.mode.name : SSticker.master_mode] ([SSticker.master_mode])")
		else
			stat("Modo de juego:", PUBLIC_GAME_MODE)
		var/extra_antags = list2params(additional_antag_types)
		stat("Antagonistas agregados:", extra_antags ? extra_antags : "None")
		stat("Voto Inicial Continua en:", "[round(config.vote_autotransfer_initial / 600, 1)] minutos")
		stat("Voto adicional cada:", "[round(config.vote_autotransfer_interval / 600, 1)] minutos")

		if(GAME_STATE <= RUNLEVEL_LOBBY)
			stat("Tiempo de empezar:", "[round(SSticker.pregame_timeleft/10)][SSticker.round_progressing ? "" : " (DELAYED)"]")
			stat("Jugadores: [totalPlayers]", "Jugadores listos: [totalPlayersReady]")
			totalPlayers = 0
			totalPlayersReady = 0
			for(var/mob/new_player/player in GLOB.player_list)
				...
				if (player.client)
					...
					if (player.client.prefs?.job_high)
						highjob = " como [player.client.prefs.job_high]"
			...
			stat("Siguiente Voto en:", "[max(round(transfer_controller.time_till_transfer_vote() / 600, 1), 0)] minutos")
...
/mob/new_player/Topic(href, href_list) // This is a full override; does not call parent.
	...
	if (href_list["show_rules"])
		client.link_url(config.rules_url, "Normas", TRUE)
		return 1
	...
	if(href_list["observe"])
		if(GAME_STATE < RUNLEVEL_LOBBY)
			to_chat(src, "<span class='warning'>Espere a que se inicie el servidor...</span>")
			return

		if(!config.respawn_delay || client.holder || alert(src,"Esta seguro de que desea observar? Tendras que esperar [config.respawn_delay] minutos antes de poder reaparecer!","Confg del jugador","Si","No") == "Si")
			if(!client)	return 1
	...
	if(href_list["late_join"])
		if(GAME_STATE != RUNLEVEL_GAME)
			to_chat(usr, SPAN_WARNING("La ronda no ha comenzado todavia o ya ha terminado."))
			return
		if (!client.holder)
			var/dsdiff = config.respawn_menu_delay MINUTES - (world.time - respawned_time)
			if (dsdiff > 0)
				to_chat(usr, SPAN_WARNING("Tu debes esperar [time2text(dsdiff, "mm:ss")] antes de reincorporarse."))
				return
...
/mob/new_player/proc/AttemptLateSpawn(var/datum/job/job, var/spawning_at)
	...
	if(!config.enter_allowed)
		to_chat(usr, "<span class='notice'>Hay un bloqueo administrativo al ingresar al juego.!</span>")
		return 0

	if(!job || !job.is_available(client))
		alert("[job.title] no esta disponible. Por favor, intente con otro.")
		return 0
	...
	if(!job || !job.is_available(client))
		to_chat(src, alert("[job.title] no esta disponible. Por favor, intente con otro."))
		return 0
	...
	if(character.mind.assigned_role == "AI")
		...
		AnnounceCyborg(character, job.title, "se ha descargado al nucleo vacío en el [character.loc.loc]")
	...
	log_and_message_admins("se ha sumado a la ronda como [character.mind.assigned_role].", character)
...
/mob/new_player/proc/AnnounceCyborg(var/mob/living/character, var/rank, var/join_message)
	if (GAME_STATE == RUNLEVEL_GAME)
		...
		GLOB.global_announcer.autosay("Un nuevo[rank ? " [rank]" : " visitor" ] [join_message ? join_message : "has arrived"].", "Ordenador de anuncio de llegadas")

/mob/new_player/proc/LateChoices()
	var/name = client.prefs.real_name

	var/list/header = list("<html><body><center>")
	header += "<b>Bienvenido, [name].<br></b>"
	header += "Duracion de la ronda: [roundduration2text()]<br>"

	if(evacuation_controller.has_evacuated())
		header += "<font color='red'><b>La [station_name()] ha sido evacuada.</b></font><br>"
	else if(evacuation_controller.is_evacuating())
		if(evacuation_controller.emergency_evacuation) // Emergency shuttle is past the point of no recall
			header += "<font color='red'>La [station_name()] se encuentra actualmente en procedimientos de evacuación.</font><br>"
		else                                           // Crew transfer initiated
			header += "<font color='red'>La [station_name()] actualmente se encuentra en tramites de transferencia de tripulación.</font><br>"

	var/list/dat = list()
	dat += "Elija entre las siguientes posiciones abiertas/validas:<br>"
	dat += "<a href='byond://?src=\ref[src];invalid_jobs=1'>[show_invalid_jobs ? "Hide":"Show"] trabajos no disponibles.</a><br>"
	dat += "<table>"
	...
	if(LAZYLEN(job_summaries))
		...
		dat += "<tr><td>No hay puestos disponibles.</td></tr>"
	// END TORCH JOBS
	...
	for(var/thing in SSmapping.submaps)
		...
		if(submap && submap.available())
			...
			if(LAZYLEN(job_summaries))
				...
				dat += "No hay puestos disponibles."
	// END SUBMAP JOBS

	dat += "</table></center>"
	if(LAZYLEN(hidden_reasons))
		var/list/additional_dat = list("<br><b>Algunas funciones se han ocultado de esta lista por los siguientes motivos:</b><br>")
...
/mob/new_player/proc/ViewManifest()
	...
	var/datum/browser/popup = new(src, "Manifiesto", "Manifiesto", 370, 420, src)
...
/mob/new_player/proc/check_species_allowed(datum/species/S, var/show_alert=1)
	if(!S.is_available_for_join() && !has_admin_rights())
		if(show_alert)
			to_chat(src, alert("Tu especie actual, [client.prefs.species], no esta disponible para jugar."))
		return 0
	if(!is_alien_whitelisted(src, S))
		if(show_alert)
			to_chat(src, alert("Actualmente no estas en la lista blanca para jugar [client.prefs.species]."))
		return 0
	return 1
...
/mob/new_player/verb/next_lobby_track()
	set name = "Reproducir pista de lobby diferente"
	set category = "OOC"
