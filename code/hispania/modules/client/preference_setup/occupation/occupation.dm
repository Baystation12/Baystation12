...
/datum/category_item/player_setup_item/occupation/content(mob/user, limit = 16, list/splitJobs, splitLimit = 1)
	...
	. += "<tt><center>"
	. += "<font size=3><b>Selecciona y configura tus preferencias de ocupacion. Las ocupaciones no disponibles estan tachadas.</b></font>"
	. += "<br>"
		...
		if(!pref.hiding_maps[job_map])
			...
			for(var/datum/job/job in map_job_list)
				...
				if(job.total_positions == 0 && job.spawn_positions == 0)
					...
					bad_message = "\[EN [(available_in_days)] DIAS]"
					...
					bad_message = "\[EDAD MINIMA: [job.minimum_character_age[bodytype]]]"
				else if(!job.is_species_allowed(S))
					bad_message = "<b>\[RESTRINGIDO POR ESPECIE]</b>"
				else if(!S.check_background(job, user.client.prefs))
					bad_message = "<b>\[RESTRINGIDO POR ANTECEDENTE]</b>"
				...
				if(pref.points_by_job[job] && (!job.available_by_default || current_level != JOB_LEVEL_NEVER))
					skill_link = "<a class = 'Points' href='?src=\ref[src];set_skills=[title]'>Habilidades</a>"
				else
					skill_link = "<a href='?src=\ref[src];set_skills=[title]'>Habilidades</a>"
				...
				if(title == GLOB.using_map.default_assistant_title)//Assistant is special
					...
					. += "<font color = '#cccccc'>No disponible al comienzo de la ronda.</font>"
				else
					var/level_link
					switch(current_level)
						if(JOB_LEVEL_LOW)
							level_link = "<font color='#cc5555'>Baja</font>"
						if(JOB_LEVEL_MEDIUM)
							level_link = "<font color='#eecc22'>Media</font>"
						if(JOB_LEVEL_HIGH)
							level_link = "<font color='#55cc55'>Alta</font>"
						else
							level_link = "<font color=black>Nunca</font>"
	...
	switch(pref.alternate_option)
		if(GET_RANDOM_JOB)
			. += "<u><a href='?src=\ref[src];job_alternative=1'>Obtenga un trabajo aleatorio si las preferencias no estan disponibles</a></u>"
		if(BE_ASSISTANT)
			. += "<u><a href='?src=\ref[src];job_alternative=1'>Ser asistente si la preferencia no esta disponible</a></u>"
		if(RETURN_TO_LOBBY)
			. += "<u><a href='?src=\ref[src];job_alternative=1'>Regrese al lobby si la preferencia no esta disponible</a></u>"
...
/datum/category_item/player_setup_item/occupation/OnTopic(href, href_list, user)
	if(href_list["reset_jobs"])
		...
		if (job)
			...
			var/choice = input("Choose an title for [job.title].", "Elija el titulo", pref.GetPlayerAltTitle(job)) as anything in choices|null
		...
		if(istype(job))
			...
			var/choice = input(user, "Elige tu rama de servicio.", CHARACTER_PREFERENCE_INPUT_TITLE) as null|anything in options
		...
		if(istype(job))
			...
			var/choice = input(user, "Elige tu rango.", CHARACTER_PREFERENCE_INPUT_TITLE) as null|anything in options
	...
	//From the skills popup
		...
		if(job.alt_titles)
			dat += "<i><b>Titulos alternativos:</b> [english_list(job.alt_titles)].</i>"
		send_rsc(user, job.get_job_icon(), "job[ckey(rank)].png")
		dat += "<img src=job[ckey(rank)].png width=96 height=96 style='float:left;'>"
		if(job.department)
			dat += "<b>Departamento:</b> [job.department]."
			if(job.head_position)
				dat += "Usted esta a cargo de este departamento."

		dat += "respondes a <b>[job.supervisors]</b> normalmente."

		if(job.allowed_branches)
			dat += "Puedes ser de los siguientes rangos:"
		...
		if(config.wiki_url)
			dat += "<a href='?src=\ref[src];job_wiki=[rank]'>Abrir pagina wiki en el navegador</a>"

		var/description = job.get_description_blurb()
		if(description)
			dat += html_encode(description)
		var/datum/browser/popup = new(user, "Informacion del trabajo", "[capitalize(rank)]", 430, 520, src)
