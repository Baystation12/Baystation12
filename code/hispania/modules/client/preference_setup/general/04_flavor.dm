...
/datum/category_item/player_setup_item/physical/flavor/content(var/mob/user)
	. += "<b>descripcion:</b><br>"
	. += "<a href='?src=\ref[src];flavor_text=open'>Texto de descripcion</a><br/>"
	. += "<a href='?src=\ref[src];flavour_text_robot=open'>Texto de descripcion de robots</a><br/>"
...
/datum/category_item/player_setup_item/physical/flavor/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["flavor_text"])
		switch(href_list["flavor_text"])
			if("open")
			if("general")
				var/msg = sanitize(input(usr,"Da una descripcion general de tu personaje. Esto se mostrara independientemente de la ropa. No incluya informacion OOC aqui.","Texto de descripcion",html_decode(pref.flavor_texts[href_list["flavor_text"]])) as message, extra = 0)
				if(CanUseTopic(user))
					pref.flavor_texts[href_list["flavor_text"]] = msg
			else
				var/msg = sanitize(input(usr,"texto de descripcion para su [href_list["flavor_text"]].","Texto de descripcion",html_decode(pref.flavor_texts[href_list["flavor_text"]])) as message, extra = 0)
				if(CanUseTopic(user))
					pref.flavor_texts[href_list["flavor_text"]] = msg
		SetFlavorText(user)
		...
		switch(href_list["flavour_text_robot"])
			if("open")
			if("Default")
				var/msg = sanitize(input(usr,"Da una descripcion general para su robot. Se utilizara para cualquier modulo sin configuracion individual.","Texto de descripcion",html_decode(pref.flavour_texts_robot["Default"])) as message, extra = 0)
				if(CanUseTopic(user))
					pref.flavour_texts_robot[href_list["flavour_text_robot"]] = msg
			else
				var/msg = sanitize(input(usr,"Texto de sabor para su robot con [href_list["flavour_text_robot"]] modulo. Si deja esto vacio, se usará el texto de sabor predeterminado para este modulo.","Texto de descripcion",html_decode(pref.flavour_texts_robot[href_list["flavour_text_robot"]])) as message, extra = 0)
				if(CanUseTopic(user))
					pref.flavour_texts_robot[href_list["flavour_text_robot"]] = msg
...
/datum/category_item/player_setup_item/physical/flavor/proc/SetFlavorText(mob/user)
	var/HTML = "<body>"
	HTML += "<tt><center>"
	HTML += "<b>Texto de descripcion</b> <hr />"
	HTML += "<br></center>"
...
/datum/category_item/player_setup_item/physical/flavor/proc/SetFlavourTextRobot(mob/user)
	var/HTML = "<body>"
	HTML += "<tt><center>"
	HTML += "<b>Texto de descripcion de robots</b> <hr />"
	HTML += "<br></center>"
	HTML += "<a href='?src=\ref[src];flavour_text_robot=Default'>Predeterminado:</a> "
