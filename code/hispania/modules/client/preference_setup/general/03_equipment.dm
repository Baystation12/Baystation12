...
/datum/category_item/player_setup_item/physical/equipment/content()
	...
	for(var/datum/category_group/underwear/UWC in GLOB.underwear.categories)
		var/item_name = (pref.all_underwear && pref.all_underwear[UWC.name]) ? pref.all_underwear[UWC.name] : "No"
		...
		. += "<br>"
	. += "Tipo de mochila: <a href='?src=\ref[src];change_backpack=1'><b>[pref.backpack.name]</b></a>"
	...
	. += "<br>"
	. += "Config del sensor del traje: <a href='?src=\ref[src];change_sensor_setting=1'>[pref.sensor_setting]</a><br />"
	. += "bloqueo del sensore del traje: <a href='?src=\ref[src];toggle_sensors_locked=1'>[pref.sensors_locked ? "Bloqueado" : "Desbloqueado"]</a><br />"
...
/datum/category_item/player_setup_item/physical/equipment/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["change_underwear"])
		...
		var/datum/category_item/underwear/selected_underwear = input(user, "Elige ropa interior:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.all_underwear[UWC.name]) as null|anything in UWC.items
		...
		var/new_backpack = input(user, "Elige el estilo de mochila:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.backpack) as null|anything in backpacks_by_name
		...
		var/switchMode = input("Seleccione un modo de sensor:", "Modo sensor de traje", pref.sensor_setting) as null | anything in SUIT_SENSOR_MODES
