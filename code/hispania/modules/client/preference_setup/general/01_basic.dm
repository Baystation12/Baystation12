...
/datum/category_item/player_setup_item/physical/basic/content()
	...
	. += "[TBTN("rename", pref.real_name, "Nombre")] [BTN("random_name", "Aleatorio")]"
	. = jointext(., null)


/datum/category_item/player_setup_item/physical/basic/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["rename"])
		var/raw_name = input(user, "Elige el nombre de tu personaje:", "Nombre del personaje")  as text|null
		if (!isnull(raw_name) && CanUseTopic(user))
			...
			if(new_name)
				...
				to_chat(user, "<span class='warning'>Nombre invalido. Debe tener al menos 2 palabras y como máximo [MAX_NAME_LEN] palabras largas. Solo puede contener los caracteres A-Z, a-z, -, ' y .</span>")
		...
		for(var/spawntype in spawntypes())
			spawnkeys += spawntype
		var/choice = input(user, "Donde te gustaria aparecer cuando te unes tarde?") as null|anything in spawnkeys
