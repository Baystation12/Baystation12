...
/datum/category_item/player_setup_item/law_pref/content()
	...
	if(!(species && species.has_organ[BP_POSIBRAIN]))
		. += "<b>Tu especie no tiene leyes</b><br>"
	else
		. += "<b>Encadenar: </b>"
		if(!pref.is_shackled)
			. += "<span class='linkOn'>Desactivado</span>"
			. += "<a href='?src=\ref[src];toggle_shackle=[pref.is_shackled]'>Activado</a>"
			. += "<br>Solo los positronicos encadenados tienen leyes en un chasis positronico integrado."
			. += "<hr>"
		else
			. += "<a href='?src=\ref[src];toggle_shackle=[pref.is_shackled]'>Desactivado</a>"
			. += "<span class='linkOn'>Activado</span>"
			. += "<br>Estas encadenado y tienes leyes que restringen tu comportamiento."
			. += "<hr>"

			. += "<b>Tus Leyes Actuales:</b><br>"

			if(!pref.laws.len)
				. += "<b>Actualmente no tienes leyes.</b><br>"
			else
				for(var/i in 1 to pref.laws.len)
					. += "[i]) [pref.laws[i]]<br>"

			. += "Conjuntos de leyes: <a href='?src=\ref[src];lawsets=1'>Cargar conjunto</a><br>"
...
/datum/category_item/player_setup_item/law_pref/OnTopic(href, href_list, user)
	if(href_list["toggle_shackle"])
		...
		// Post selection
		var/chosen_lawset = input(user, "Elija un conjunto de leyes:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.laws)  as null|anything in valid_lawsets
		if(chosen_lawset)
