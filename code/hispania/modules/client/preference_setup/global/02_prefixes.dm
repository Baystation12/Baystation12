...
/datum/category_item/player_setup_item/player_global/prefixes/content(var/mob/user)
	...
	for(var/prefix_name in prefix_by_name)
		...
		if(prefix_instance.is_locked)
			. += "<span class='linkOff'>Cambiar</span>"
		else

			. += "<a href='?src=\ref[src];change_prefix=\ref[prefix_instance]'>Cambiar</a>"

		. += "</td><td>"

		if(prefix_instance.is_locked || current_prefix == prefix_instance.default_key)
			. += "<span class='linkOff'>Reiniciar</span>"
		else
			. += "<a href='?src=\ref[src];reset_prefix=\ref[prefix_instance]'>Reiniciar</a>"
		. += "</td></tr>"
	. += "</table>"
...
/datum/category_item/player_setup_item/player_global/prefixes/OnTopic(var/href, var/list/href_list, var/mob/user)
	if(href_list["change_prefix"])
		...
		do
			...
			var/new_key = input(user, "Ingrese un solo caracter especial. Los siguientes caracteres ya estan en uso como prefijos: [jointext(keys_in_use, " ")]", CHARACTER_PREFERENCE_INPUT_TITLE, pref.prefix_keys_by_type[prefix_instance.type]) as null|text
			if(!new_key || new_key == pref.prefix_keys_by_type[prefix_instance.type] || !CanUseTopic(user))
				return TOPIC_NOACTION

			if(length(new_key) != 1)
				alert(user, "Solo se permiten caracteres individuales.", "Error", "Ok")
			else if(contains_az09(new_key))
				alert(user, "Solo se permiten caracteres especiales.", "Error", "Ok")
			else if(new_key == " ")
				alert(user, "El caracter de espacio no esta permitido.", "Error", "Ok")
			else
				...
				if(pref.prefix_keys_by_type[prefix_instance.type] != new_key)
					alert(user, "La clave seleccionada ya es la clave predeterminada para otro prefijo.", "Error", "Ok")
				else
