...
/datum/category_item/player_setup_item/loadout/content()
	...
	if(config.max_gear_cost < INFINITY)
		. += "<b><font color = '[fcolor]'>[total_cost]/[config.max_gear_cost]</font> puntos de equipamiento gastados.</b>"

	. += "<a href='?src=\ref[src];clear_loadout=1'>Borrar equipamiento</a>"
	. += "<a href='?src=\ref[src];toggle_hiding=1'>[hide_unavailable_gear ? "Mostrar todo" : "Ocultar no disponible"]</a></center></td></tr>"
...
/datum/gear/New()
	if(HAS_FLAGS(flags, GEAR_HAS_TYPE_SELECTION|GEAR_HAS_SUBTYPE_SELECTION))
		CRASH("Es posible que no tenga ajustes de seleccion de tipo y subtipo")
	if(!description)
		var/obj/O = path
...
/datum/gear/proc/spawn_in_storage_or_drop(mob/living/carbon/human/subject, metadata)
	...
	if (istype(item, /obj/item/organ/internal/augment))
		...
		if (!parent)
			to_chat(subject, SPAN_WARNING("No se pudo encontrar un organo valido para instalar el [augment] dentro!"))
			qdel(augment)
			return
		...
		if (augment.surgery_configure(subject, subject, parent, null, surgery_step))
			to_chat(subject, SPAN_WARNING("No se pudo configurar el [augment] para la instalación en su [parent.name]!"))
			qdel(augment)
			return
	...
	if (container)
		to_chat(subject, SPAN_NOTICE("Colocando el [item] en tu [container.name]!"))
	else if (subject.equip_to_appropriate_slot(item))
		to_chat(subject, SPAN_NOTICE("Colocando el [item] en tu inventario!"))
	else if (subject.put_in_hands(item))
		to_chat(subject, SPAN_NOTICE("Colocando el [item] en tus manos!"))
	else
		to_chat(subject, SPAN_WARNING("Soltando el [item] en el piso!"))
