...
/datum/category_item/player_setup_item/antagonism/candidacy/content(var/mob/user)
	. = list()
	. += "<b>Roles especiales habilitados:</b><br>"
	. += "<table>"
	...
	for(var/antag_type in all_antag_types)
		...
		if(jobban_isbanned(preference_mob(), antag.id) || (antag.id == MODE_MALFUNCTION && jobban_isbanned(preference_mob(), "AI")))
			...
			. += "<span class='linkOn'>Alto</span> <a href='?src=\ref[src];add_maybe=[antag.id]'>Bajo</a> <a href='?src=\ref[src];del_special=[antag.id]'>Nunca</a></br>"
		else if(antag.id in pref.may_be_special_role)
			. += "<a href='?src=\ref[src];add_special=[antag.id]'>Alto</a> <span class='linkOn'>Bajo</span> <a href='?src=\ref[src];del_special=[antag.id]'>Nunca</a></br>"
		else
			. += "<a href='?src=\ref[src];add_special=[antag.id]'>Alto</a> <a href='?src=\ref[src];add_maybe=[antag.id]'>Bajo</a> <span class='linkOn'>Nunca</span></br>"
	...
	if (BE_PAI in pref.be_special_role)
		. += "<td><span class='linkOn'>Si</span> <a href='?src=\ref[src];del_special=[BE_PAI]'>No</a></br></td></tr>"
	else
		. += "<td><a href='?src=\ref[src];add_special=[BE_PAI]'>Si</a> <span class='linkOn'>No</span></br></td></tr>"
	. += "</table>"
	. = jointext(.,null)
