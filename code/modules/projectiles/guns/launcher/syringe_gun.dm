...
/obj/item/weapon/syringe_cartridge/throw_impact(atom/hit_atom, var/speed)
	...
	if(syringe)
		...
		if(speed >= 10 && isliving(hit_atom))
			...
			var/assumed_hit_zone = ran_zone()
			if(L.can_inject(null, assumed_hit_zone) && syringe.reagents)
				if(!(L.run_armor_check(assumed_hit_zone, "melee", 35) == 100)) //35 Armour pen because it's the melee armour pen
