/obj/item/weapon/gun/energy/advanced/proc/medical_scan(var/mob/living/carbon/human/H)
	if(H)
		if(intelligun_status & INTELLIGUN_ANALYZING)
			src.speak("\red Processing previous analysis! Please wait.")
			return
		if(prob(1))
			reliability -= 1
		if(get_dist(src, H) > 3)
			src.speak("\red Unable to initiate scan. Distance too great.")
			return
		if(shutdown)
			return
		if(medical_data.len >= 50)
			src.speak("\blue Removing previous medical records..")
			medical_data.Cut()
		src.speak("\blue <b>Initating scan for: [H]</b>")
		if(!istype(H))
			src.speak("\red Scan results inconclusive")
			return
		intelligun_status |= INTELLIGUN_ANALYZING
		spawn(15)
			if(!can_use_charge(60)) return
			src.speak("\blue Current Status: [H.stat > 1 ? "\red dead" : "\green [H.health]% \blue healthy"]")
			medical_data += "<b>Scan Results for: [H]</b><br><hr><br>"
			medical_data += "\blue Overall Status: [H.stat > 1 ? "\red dead" : "\green [H.health]% \blue healthy"]<br>"
			if(H.health < 75)
				if(H.stat <= 1)
					src.speak("\blue Beginning detailed analysis of data..")
					spawn(rand(10, 25))
						src.speak("\blue Measurements taken. Estimating symptoms...")
					spawn(rand(25, 50))
						src.speak("\blue Symptoms estimated. Formulating..")
					spawn(rand(50, 60))
						var/diagnosis = "\blue Diagnosis Complete. Measurements"
						var/symptoms = "\blue Symptoms may include"
						...
						if(H:vessel)
							...
							if(blood_volume < 560)
								diagnosis += ": \blue Approximate Blood Percentage :: [blood_percent < 50 ? "\red [rand(blood_percent / 2, blood_percent)]" : "<font color=#808000> [rand(blood_percent / 2, blood_percent)]</font>"]% "
						...
						symptoms += ":\blue  Recommendation: See a doctor!"
...
/obj/item/weapon/gun/energy/advanced/proc/analyze(var/mob/user, var/message as text)
	...
	if(!target || !istype(target))
		...
		if(!target)
			if(!ai)
				src.speak("\red Alert: Automatic AI recognition software unresponsive!")
				return
			src.speak("\blue Unable to find anyone in range to analyze!")
			return
	if(!(target in view(3, T)))
		src.speak("\blue [target] is too far away!")
		return
	medical_scan(target)
...
/obj/item/weapon/gun/energy/advanced/proc/lock_weapon()
	...
	src.speak("\blue Controls [intelligun_status & INTELLIGUN_LOCKED ? "" : "un"]locked")
...
/obj/item/weapon/gun/energy/advanced/proc/arrest(var/mob/user, var/message as message, var/forceful = 0)
	...
	if(forceful)
		src.speak("\red [target], you are officially under arrest! You are required to lay face-down on the floor within \
				   the next ten seconds or use of force may be authorised! Failure to do as requested by \
				   [owner ? "[owner]" : "your arresting officer"] will result in further charges against you!")
		spawn(50)
			if(!target.stat || !target.lying || !target.restrained())
				src.speak("\red [target], you have five seconds to submit!")
				spawn(50)
					if(!target.stat || !target.lying || !target.restrained())
						override_safety = 1
						override_safety()
		...
		if(minor_crimes == "" && major_crimes == "")
			src.speak("\red You can't arrest them without reason!")
			return
		if(major_crimes	!= "")
			if(minor_crimes != "")
				...
				if(E.fields["name"] == temp)
					for (var/datum/data/record/R in data_core.security)
						if(R.fields["id"] == E.fields["id"])
							...
							return
			src.speak("\red Error. Unable to find associated security record.")

/obj/item/weapon/gun/energy/advanced/proc/shutdown_weapon()
	if(shutdown == -1) return
	shutdown = -1
	src.speak("\blue Beginning shutdown..", 0, 1)
	spawn(rand(20, 450))
		src.speak("\red Shutdown complete.", 0, 1)
		shutdown = 1

/obj/item/weapon/gun/energy/advanced/proc/activate_weapon()
	if(shutdown == -1) return
	if(!shutdown)
		src.speak("\blue I'm already active!", 0, 1)
		return
	src.speak("\blue Beginning startup..", 0, 1)
	shutdown = -1
	spawn(rand(20, 450))
		if(prob(100 - reliability))
			src.speak("\red Error.", 0, 1)
			...
			src.speak("\green Startup complete!", 0, 1)
			shutdown = 0

/obj/item/weapon/gun/energy/advanced/proc/record()
	if(recording == 2 || intelligun_status & INTELLIGUN_BACKUP_POWER)
		src.speak("\red Recording is disabled!")
		return
	if(timerecorded >= 15)
		src.speak("\blue Unable to record more data: Memory slot is at it's maximum capacity!")
		return
	if(shutdown) return
	src.speak("\blue Recording [recording ? "stopped" : "started"]")
...
/obj/item/weapon/gun/energy/advanced/proc/play()
	if(held_pai)
		if(!(intelligun_status & INTELLIGUN_AI_ENABLED))
			src.speak("\red Unable to play recording: AI disabled!")
			return
	if(shutdown) return
	if(intelligun_status & INTELLIGUN_PLAYING)
		src.speak("\blue You are already playing!")
		return
	if(recording)
		src.speak("\blue You cannot play whilst recording!")
		return
	src.speak("\blue Playing recorded data...")
...
/obj/item/weapon/gun/energy/advanced/proc/flashlight()
	...
	if(intelligun_status & INTELLIGUN_BACKUP_POWER && !(intelligun_status & INTELLIGUN_FLASHLIGHT))
		src.speak("\red This feature is disabled due to power loss!")
		return
...
/obj/item/weapon/gun/energy/advanced/proc/backup()
	...
	src.speak("\red This is [owner ? "[owner.name]" : "[ai_name]"] requesting backup at the location of: [location] immediatley.", 1)
...
/obj/item/weapon/gun/energy/advanced/proc/override_safety()
	if(override_safety == 1)
		override_safety = 0
		src.speak("\red <b>Safety protocols temporarily disengaged. Approximately ten shots before safety re-engages.</b>")
	else if(override_safety == 3)
		override_safety = 0
	else if(override_safety == 2)
		src.speak("\blue Unable to disengage safety protocols without reason.")
	else if(override_safety == 0)
		src.speak("\blue Engaging safety protocols..")
		spawn(rand(10, 50))
			override_safety = 2
	shotcount = 0
...
/obj/item/weapon/gun/energy/advanced/proc/supercharge()
	...
	if(src.intelligun_status & INTELLIGUN_SUPERCHARGED)
		src.speak("\blue Battery temperature returning to optimal levels.")
		intelligun_status &= ~INTELLIGUN_SUPERCHARGED
		return
	else
		src.speak("\red Ov^#rload¤i-ing re-BZZT")
		intelligun_status &= INTELLIGUN_SUPERCHARGED
...
/obj/item/weapon/gun/energy/advanced/proc/commands()
	src.speak("\blue Listing commands:	<br>\
