...
/datum/ghosttrap/proc/request_player(mob/target, request_string, request_timeout)
	if(request_timeout)
		...
		GLOB.destroyed_event.register(target, src, /datum/ghosttrap::unregister_target())
	else
...
/datum/ghosttrap/proc/unregister_target(target)
	...
	GLOB.destroyed_event.unregister(target, src, /datum/ghosttrap::unregister_target())
