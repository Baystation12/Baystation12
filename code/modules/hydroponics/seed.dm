...
/datum/seed/proc/harvest(var/mob/user,var/yield_mod,var/harvest_sample,var/force_amount)
	...
	if(!force_amount && get_trait(TRAIT_YIELD) == 0 && !harvest_sample)
		to_chat(if(istype(user)) user, "<span class='danger'>You fail to harvest anything useful.</span>")
	else
		to_chat(if(istype(user)) user, "You [harvest_sample ? "take a sample" : "harvest"] from the [display_name].")
