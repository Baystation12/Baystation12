...
/obj/effect/vine/proc/manual_unbuckle(mob/user)
	...
	if(buckled_mob != user)
		...
		if(do_after(user, breakouttime, src, DO_DEFAULT | DO_SHOW_PROGRESS | DO_USER_UNIQUE_ACT, incapacitation_flags = INCAPACITATION_DEFAULT & ~INCAPACITATION_RESTRAINED))
			if(unbuckle_mob())
				user.visible_message(
