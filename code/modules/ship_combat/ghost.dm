...
/mob/living/verb/ghost()
	...
	if(stat == DEAD)
		announce_ghost_joinleave(ghostize(0))
		...
		var/mob/observer/ghost/ghost = ghostize(1)	//0 parameter is so we can never re-enter our body, "Charlie, you can never come baaaack~" :3
...
*/
	stop_following()
	var/turf/T = pick_area_turf(thearea.type)
	if(T)
		usr.loc = T
	else
		usr << "<span class='warning'>You cannot teleport there!</span>"
...
/mob/observer/ghost/AltClickOn(var/atom/A)
	A.ghost_interact(src)
	return

/atom/proc/ghost_interact(var/mob/user)
	world << "Ghost interact!"
	return
