...
/decl/psionic_power/coercion/mindread/invoke(var/mob/living/user, var/mob/living/target)
	...
	if(!question || user.incapacitated() || !do_after(user, 20, target, DO_DEFAULT & ~DO_TARGET_UNIQUE_ACT))
		return TRUE
...
/decl/psionic_power/coercion/mindslave/invoke(var/mob/living/user, var/mob/living/target)
	...
	if(.)
		...
		if(!do_after(user, target.stat == CONSCIOUS ? 80 : 40, target, DO_DEFAULT & ~DO_TARGET_UNIQUE_ACT))
...
/decl/psionic_power/coercion/assay/invoke(var/mob/living/user, var/mob/living/target)
	...
	if(.)
		...
		if(!do_after(user, (target.stat == CONSCIOUS ? 50 : 25), target, DO_DEFAULT & ~DO_TARGET_UNIQUE_ACT))
