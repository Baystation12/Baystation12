...
/obj/item/device/assembly_holder/Topic(href, href_list)
			...
			if(index % 2)
				...
				for(var/obj/item/device/assembly/A in connected_devices)
					if(!A.holder_interface())
						...
						break
				if(failed)
					usr << "\red That device's interface is electronically locked!"
				else if(opened)
	...
	if(href_list["connect"])
		if(drawing_connection)
			usr << "\blue You stop drawing connections!"
			drawing_connection = null
		else
			if(admin_access(usr))
				usr << "\blue You begin drawing connections!"
	...
	if(href_list["connect_to"])
		...
		if(failed)
			...
			usr << "\red You cannot link an object to itself!"
		else if(first.connects_to.Find(index) || first.connects_to.Find(index))
			usr << "\red They are already connected!"
...
//					first.connects_to.Add(index)
			...
			first.connects_to.Add(index)
			usr << "\blue You successfully link [first] and [second]!"
		drawing_connection = null
