...
/obj/item/device/assembly/scanner/healthanalyzer/proc/scan(var/mob/living/M as mob, mob/living/user as mob)
	...
	if ((CLUMSY in user.mutations) && prob(50))
		...
		scanned_data.Add("<span class='notice'>Analyzing Results for The floor:</span>", 1)
		scanned_data.Add("<span class='notice'>Overall Status: Healthy</span>", 1)
		scanned_data.Add("<span class='notice'>    Damage Specifics: 0-0-0-0</span>", 1)
		scanned_data.Add("<span class='notice'>Key: Suffocation/Toxin/Burns/Brute</span>", 1)
		scanned_data.Add("<span class='notice'>Body Temperature: ???</span>", 1)
		return
	...
	if (!istype(M,/mob/living/carbon/human) || M.isSynthetic())
		...
		scanned_data.Add("<span class='notice'>    Key: <font color='blue'>Suffocation</font>/<font color='green'>Toxin</font>/<font color='#FFA500'>Burns</font>/<font color='red'>Brute</font></span>", 1)
		scanned_data.Add("<span class='notice'>    Damage Specifics: <font color='blue'>?</font> - <font color='green'>?</font> - <font color='#FFA500'>?</font> - <font color='red'>?</font></span>")
		scanned_data.Add("<span class='notice'>Body Temperature: [M.bodytemperature-T0C]&deg;C ([M.bodytemperature*1.8-459.67]&deg;F)</span>", 1)
		scanned_data.Add("<span class='warning'>Warning: Blood Level ERROR: --% --cl.</span> <span class='notice'>Type: ERROR</span>")
	...
	if(M.status_flags & FAKEDEATH)
		...
		scanned_data.Add("<span class='notice'>Analyzing Results for [M]:</span>")
		scanned_data.Add("<span class='notice'>Overall Status: dead</span>")
	else
	...
	scanned_data.Add("<span class='notice'>    Key: <font color='blue'>Suffocation</font>/<font color='green'>Toxin</font>/<font color='#FFA500'>Burns</font>/<font color='red'>Brute</font></span>", 1)
	scanned_data.Add("<span class='notice'>    Damage Specifics: <font color='blue'>[OX]</font> - <font color='green'>[TX]</font> - <font color='#FFA500'>[BU]</font> - <font color='red'>[BR]</font></span>")
	scanned_data.Add("<span class='notice'>Body Temperature: [M.bodytemperature-T0C]&deg;C ([M.bodytemperature*1.8-459.67]&deg;F)</span>", 1)
	...
	if(istype(M, /mob/living/carbon/human) && mode == 1)
		...
		scanned_data.Add("<span class='notice'>Localized Damage, Brute/Burn:</span>",1)
		if(length(damaged)>0)
			for(var/obj/item/organ/external/org in damaged)
				scanned_data.Add(text("<span class='notice'>     [][]: [][] - []</span>",
				capitalize(org.name),
			...
			scanned_data.Add("<span class='notice'>    Limbs are OK.</span>",1)
	...
	if(istype(M, /mob/living/carbon))
		...
		if(C.reagents.total_volume)
			...
			if(unknown)
				scanned_data.Add("<span class='warning'>Warning: Unknown substance[(unknown>1)?"s":""] detected in subject's blood.</span>")
		...
		if(C.virus2.len)
			for (var/ID in C.virus2)
				if (ID in virusDB)
					...
					scanned_data.Add("<span class='warning'>Warning: Pathogen [V.fields["name"]] detected in subject's blood. Known antigen : [V.fields["antigen"]]</span>")
//			scanned_data.Add(text("<span class='warning'>Warning: Unknown pathogen detected in subject's blood.</span>"))
	if (M.getCloneLoss())
		scanned_data.Add("<span class='warning'>Subject appears to have been imperfectly cloned.</span>")
//	if (M.reagents && M.reagents.get_reagent_amount("inaprovaline"))
//		scanned_data.Add("<span class='notice'>Bloodstream Analysis located [M.reagents:get_reagent_amount("inaprovaline")] units of rejuvenation chemicals.</span>")
	if (M.has_brain_worms())
		...
		scanned_data.Add("<span class='warning'>Severe brain damage detected. Subject likely to have a traumatic brain injury.</span>")
	...
	if(ishuman(M))
		...
		for(var/name in H.organs_by_name)
			...
			if(e && e.status & ORGAN_BROKEN)
				scanned_data.Add(text("<span class='warning'>Bone fractures detected. Advanced scanner required for location.</span>"), 1)
				break
		for(var/obj/item/organ/external/e in H.organs)
			if(!e)
				continue
			for(var/datum/wound/W in e.wounds) if(W.internal)
				scanned_data.Add(text("<span class='warning'>Internal bleeding detected. Advanced scanner required for location.</span>"), 1)
				break
		if(M:vessel)
			...
			if((blood_percent <= BLOOD_VOLUME_SAFE) && (blood_percent > BLOOD_VOLUME_BAD))
				scanned_data.Add("<span class='danger'>Warning: Blood Level LOW: [blood_percent]% [blood_volume]cl.</span> <span class='notice'>Type: [blood_type]</span>")
			else if(blood_percent <= BLOOD_VOLUME_BAD)
				scanned_data.Add("<span class='danger'><i>Warning: Blood Level CRITICAL: [blood_percent]% [blood_volume]cl.</i></span> <span class='notice'>Type: [blood_type]</span>")
			else
