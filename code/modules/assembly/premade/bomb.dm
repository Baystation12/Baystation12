/obj/item/device/onetankbomb
	...
	var/obj/item/device/assembly_holder/frame/bombassembly = null   //The first part of the bomb is an assembly holder, holding an igniter+some device
...
/obj/item/weapon/tank/proc/bomb_assemble(W,user)	//Bomb assembly proc. This turns assembly+tank into a bomb
	...
	var/mob/M = user
	var/igniter_count = 0
	for(var/obj/item/device/assembly/A in S.connected_devices)
		if(istype(A, /obj/item/device/assembly/igniter))		//Check if either part of the assembly has an igniter, but if both parts are igniters, then fuck it
			igniter_count++
	if(!igniter_count || igniter_count == 2)
		return
