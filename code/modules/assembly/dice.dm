...
/obj/item/device/assembly/dice/activate()
	send_data(list(pick(to_pick)))
	return 1

/obj/item/device/assembly/dice/signal_failure(var/sent = 0)
	if(sent && prob(50))
		send_data(list(pick("1", "59", "!%ùËr", "brumm", "Oesophagus", "why", "undergraduate tomato")))

/obj/item/device/assembly/dice/get_data()
	var/list/data = list()
	for(var/i=1,i<=to_pick.len,i++)
		data.Add("Random [i]", to_pick[i])
	return data

/obj/item/device/assembly/dice/get_buttons()
	var/list/data = list()
	if(to_pick.len < 5)
		data.Add("Add Random")
	return data

/obj/item/device/assembly/dice/Topic(href, href_list)
	if(href_list["option"])
		for(var/i=1,i<=to_pick.len,i++)
