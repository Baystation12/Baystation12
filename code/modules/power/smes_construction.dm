...
/obj/machinery/power/smes/buildable/proc/total_system_failure(var/intensity = 0, var/mob/user as mob)
	...
	if(h_user.gloves)
		...
		if(G.siemens_coefficient == 0)
			user_protected = 1
	log_and_message_admins("SMES FAILURE: <b>[src.x]X [src.y]Y [src.z]Z</b> User: [usr.ckey], Intensity: [intensity]/100 - <A HREF='?_src_=holder;admin_goto=1;X=[src.x];Y=[src.y];Z=[src.z]'>JMP</a>")
