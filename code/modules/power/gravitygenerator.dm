...
/obj/machinery/computer/gravity_control_computer/attack_hand(mob/user as mob)
	...
	if(gravity_generator)
		if(gravity_generator:on)
			dat += "<font colour=green><br><tt>Gravity Status: ON</tt></font><br>"
		else
			dat += "<font colour=red><br><tt>Gravity Status: OFF</tt></font><br>"

		dat += "<br><tt>Currently Supplying Gravitons To:</tt><br>"

		for(var/area/A in gravity_generator:localareas)
			if(A.has_gravity && gravity_generator:on)
				dat += "<tt><font colour=green>[A]</tt></font><br>"

			else if (A.has_gravity)
				dat += "<tt><font colour=yellow>[A]</tt></font><br>"

			else
				dat += "<tt><font colour=red>[A]</tt></font><br>"

		dat += "<br><tt>Maintainence Functions:</tt><br>"
		if(gravity_generator:on)
			dat += "<a href='byond://?src=\ref[src];gentoggle=1'><font colour=red> TURN GRAVITY GENERATOR OFF. </font></a>"
		else
			dat += "<a href='byond://?src=\ref[src];gentoggle=1'><font colour=green> TURN GRAVITY GENERATOR ON. </font></a>"
