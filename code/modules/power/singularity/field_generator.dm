	...
	if(state == 2)
		if(get_dist(src, user) <= 1)//Need to actually touch the thing to turn it on
			if(src.active >= 1)
				...
				turn_on()
				investigate_log("<font colour='green'>activated</font> by [user.key].","singulo")
...
/obj/machinery/field_generator/proc/calc_power()
	...
	if(draw_power(round(power_draw/2,1)))
		...
		turn_off()
		investigate_log("ran out of power and <font colour='red'>deactivated</font>","singulo")
		src.power = 0
		return 0
		...
		for(var/obj/machinery/singularity/O in machines)
			if(O.last_warning && temp)
				if((world.time - O.last_warning) > 50) //to stop message-spam
					...
					investigate_log("has <font colour='red'>failed</font> whilst a singulo exists.","singulo")
