...
/obj/machinery/artifact/Process()
	...
	if(pulledby)
		check_triggers( /datum/artifact_trigger::on_touch(), pulledby)

	var/datum/gas_mixture/enivonment = loc.return_air()
	if(enivonment.return_pressure() >= SOUND_MINIMUM_PRESSURE)
		check_triggers( /datum/artifact_trigger::on_gas_exposure(), enivonment)
...
/obj/machinery/artifact/attack_robot(mob/living/user)
	if(!CanPhysicallyInteract(user))
		return
	check_triggers( /datum/artifact_trigger::on_touch(), user)
...
/obj/machinery/artifact/attackby(obj/item/W, mob/living/user)
	. = ..()
	check_triggers( /datum/artifact_trigger::on_hit(), W, user)

/obj/machinery/artifact/Bumped(M)
	..()
	check_triggers( /datum/artifact_trigger::on_bump(), M)

/obj/machinery/artifact/bullet_act(obj/item/projectile/P)
	check_triggers( /datum/artifact_trigger::on_hit(), P)

/obj/machinery/artifact/ex_act(severity)
	if(check_triggers( /datum/artifact_trigger::on_explosion(), severity))
		return
	switch(severity)
...
/obj/machinery/artifact/water_act(depth)
	check_triggers( /datum/artifact_trigger::on_water_act(), depth)
