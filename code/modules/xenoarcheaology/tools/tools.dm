			...
			if(prob(scan_ticks * 10))
				spawn(0)
					...
					if(GLOB.processing_objects.Find(src))
						...
						var/turf/T = get_turf(src)
						if(target_radio)
							T.visible_message("[icon2html(src, world)] [src] [pick("chirps","chirrups","cheeps")] happily.")
						else
							T.visible_message("[icon2html(src, world)] [src] [pick("chirps","chirrups","cheeps")] sadly.")
		else
			icon_state = "pinoff"
