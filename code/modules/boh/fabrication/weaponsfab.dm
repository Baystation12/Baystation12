/obj/machinery/weapons_fabricator
	...
	desc = "A modified fabricator designed to produce weapons instead of common items."
...
/obj/machinery/weapons_fabricator/Initialize()
	files = new /datum/research(src) //Setup the research data holder.
	update_categories()
	. = ..()

/obj/machinery/weapons_fabricator/Process()
	if(stat)
		return
	if(busy)
...
/obj/machinery/weapons_fabricator/on_update_icon()
	overlays.Cut()
	if(panel_open)
		icon_state = "fab-o"
	else
...
/obj/machinery/weapons_fabricator/dismantle()
...
/obj/machinery/weapons_fabricator/ui_interact(var/mob/user, var/ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	var/data[0]
	...
	if(!ui)
		ui = new(user, src, ui_key, "weaponsfab.tmpl", "Weapons Fabricator UI", 800, 600)
		ui.set_initial_data(data)
		ui.open()
		ui.set_auto_update(1)

/obj/machinery/weapons_fabricator/Topic(href, href_list)
	if(..())
		return
...
/obj/machinery/weapons_fabricator/components_are_accessible(path)
	return !busy && ..()

/obj/machinery/weapons_fabricator/cannot_transition_to(state_path)
	if(busy)
		return SPAN_NOTICE("\The [src] is busy. Please wait for completion of previous operation.")
	return ..()

/obj/machinery/weapons_fabricator/attackby(var/obj/item/I, var/mob/user)
	if(busy)
...
/obj/machinery/weapons_fabricator/emag_act(var/remaining_charges, var/mob/user)
	switch(emagged)
		if(0)
			emagged = 0.5
...
/obj/machinery/weapons_fabricator/proc/update_busy()
	if(queue.len)
...
/obj/machinery/weapons_fabricator/proc/add_to_queue(var/index)
	var/datum/design/D = files.known_designs[index]
	queue += D
	update_busy()

/obj/machinery/weapons_fabricator/proc/remove_from_queue(var/index)
	if(index == 1)
		progress = 0
	queue.Cut(index, index + 1)
	update_busy()

/obj/machinery/weapons_fabricator/proc/can_build(var/datum/design/D)
	for(var/M in D.materials)
		if(materials[M] <= D.materials[M] * mat_efficiency)
			return 0
	return 1

/obj/machinery/weapons_fabricator/proc/check_build()
	if(!queue.len)
		progress = 0
		return
...
/obj/machinery/weapons_fabricator/proc/get_materials()
	. = list()
	for(var/T in materials)
		. += list(list("mat" = capitalize(T), "amt" = materials[T]))

/obj/machinery/weapons_fabricator/proc/eject_materials(var/material, var/amount) // 0 amount = 0 means ejecting a full stack; -1 means eject everything
...
/obj/machinery/weapons_fabricator/proc/sync()
