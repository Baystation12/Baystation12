...
/spell/targeted/ethereal_jaunt/cast(list/targets) //magnets, so mostly hardcoded
	for(var/mob/living/target in targets)
		...
		spawn(0)
			...
			addtimer(new Callback(src, src::start_reappear(), target), duration)
...
/spell/targeted/ethereal_jaunt/proc/start_reappear(mob/living/user)
	...
	addtimer(new Callback(src, src::reappear(), mob_loc, user), reappear_duration)
...
/obj/effect/dummy/spell_jaunt/relaymove(mob/user, direction)
	...
	canmove = 0
	addtimer(new Callback(src, src::allow_move()), 2)
