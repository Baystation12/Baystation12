...
//Some simple descriptors for breaches. Global because lazy, TODO: work out a better way to do this. -- Now double the ugly with helmet breaching.

var/global/list/breach_brute_descriptors_torso = list(
	"tiny puncture",
	"ragged tear",
	"large split",
	"huge tear",
...
var/global/list/breach_burn_descriptors_torso = list(
	"small burn",
	"melted patch",
	"sizable burn",
	"large scorched area",
...
var/global/list/breach_brute_descriptors_helmet= list(
	"tiny puncture",
	"ragged tear",
	"large split",
	"huge tear",
...
var/global/list/breach_burn_descriptors_helmet = list(
	"small burn",
	"melted patch",
	"sizable burn",
	"large scorched area",
...
/datum/breach/proc/update_descriptor_torso()
	...
	if(damtype == BURN)
		descriptor = breach_burn_descriptors_torso[class]
	else if(damtype == BRUTE)
		descriptor = breach_brute_descriptors_torso[class]

/datum/breach/proc/update_descriptor_helmet()

	//Sanity...
	//Apply the correct descriptor.
	if(damtype == BURN)
		descriptor = breach_burn_descriptors_helmet[class]
	else if(damtype == BRUTE)
		descriptor = breach_brute_descriptors_helmet[class]
...
//Repair a certain amount of brute or burn damage to the suit.
/obj/item/clothing/head/helmet/space/proc/repair_breaches(var/damtype, var/amount, var/mob/user)
	...
	for(var/datum/breach/B in valid_breaches)
		...
		if(B.class <= amount_left)
			...
			B.class	-= amount_left
			amount_left = 0
			B.update_descriptor_helmet()

	user.visible_message("<b>[user]</b> patches some of the damage on \the [src].")
	calc_breach_damage()

/obj/item/clothing/head/helmet/space/proc/create_breaches(var/damtype, var/amount)
...
//Calculates the current extent of the damage to the suit.
/obj/item/clothing/head/helmet/space/proc/calc_breach_damage()
