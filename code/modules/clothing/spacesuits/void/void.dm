...
/obj/item/clothing/suit/space/void/attackby(obj/item/W, mob/living/user)
	if(!istype(user))
		return
...
/obj/item/clothing/suit/space/void/proc/ModifySuit(/obj/item/weapon/screwdriver/W, mob/living/user)
	set waitfor = FALSE
	if(user.get_inventory_slot(src) == slot_wear_suit)
		to_chat(user, "<span class='warning'>You cannot modify \the [src] while it is being worn.</span>")
		return
	...
	if(helmet || boots || tank)
		var/choice = input(user, "Which component would you like to remove?") as null|anything in list(helmet,boots,tank)
