...
/obj/item/weapon/rig/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/nano_state = GLOB.inventory_state)
	...
	if(wearer) //Internals below!!!
		...
		if(!wearer.internal || wearer.internal == air_supply)	// if they have no active internals or if tank is current internal
			...
			else if(wearer.head && (wearer.head.item_flags & ITEM_FLAG_AIRTIGHT)) // Make sure they have a head lol
				data["maskConnected"] = 1
			else
				data["maskConnected"] = 0

	data["tankPressure"] = air_supply ? round(air_supply.air_contents && air_supply.air_contents.return_pressure() ? air_supply.air_contents.return_pressure() : 0) : 0
	data["releasePressure"] = air_supply ? round(air_supply.distribute_pressure ? air_supply.distribute_pressure : 0) : 0
