...
/obj/item/weapon/book/manual/rust_engine/New()
	...
	dat = {"<html>
				...
				<li>Go into the control room, interact with the fusion core control console. Turn the field on and raise the field strength to no higher than 50 tesla. Any higher and the field will start to cause EMPs and break the machinery of the RUST, rendering it inoperable. If set much smaller, the fuel pellets might miss.</li>
				<li>Interact with the gyrotron control computer, set the power to 20, and turn it on. This will start increasing the plasma temperature to the point where reactions can occur. As the gyrotron has a high power draw, adjust it to the minimum required after reactor startup.</li>
