...
/obj/item/weapon/book/manual/New()
	...
	if(url)		// URL provided for this manual
		...
		if(config.wikiurl && !findtextEx_char(url, config.wikiurl, 1, length(config.wikiurl)+1))
			// If we have wikiurl, but it hasn't "index.php" then add it and making full link in url
			if(config.wikiurl && !findtextEx_char(config.wikiurl, "/index.php", -10))
				if(findtextEx_char(config.wikiurl, "/", -1))
