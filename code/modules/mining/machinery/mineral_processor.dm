...
/obj/machinery/mineral/processing_unit/Process()
	...
	if(output_turf)
		...
		for(var/metal in ores_stored)
			...
			var/datum/material/M = SSmaterials.get_material_by_name(metal)
...
/obj/machinery/mineral/processing_unit/proc/attempt_smelt(datum/material/metal, max_result)
	. = clamp(Floor(ores_stored[metal.name]/metal.units_per_sheet),1,max_result)
	ores_stored[metal.name] -= . * metal.units_per_sheet
	var/datum/material/M = SSmaterials.get_material_by_name(metal.ore_smelts_to)
	if(istype(M))
		M.place_sheet(output_turf, .)
	else
		. = -(.)

/obj/machinery/mineral/processing_unit/proc/attempt_compression(datum/material/metal, max_result)
	...
	if(making >= 2)
		...
		. = Floor(making * 0.5)
		var/datum/material/M = SSmaterials.get_material_by_name(metal.ore_compresses_to)
		if(istype(M))
			M.place_sheet(output_turf, .)
		else
			. = -(.)
...
/obj/machinery/mineral/processing_unit/get_console_data()
	...
	for(var/ore in ores_processing)
		...
		var/datum/material/M = SSmaterials.get_material_by_name(ore)
