...
/obj/machinery/fabricator/replicator/hear_talk(mob/M, text, verb, datum/language/speaking)
	...
	if(findtext(true_text, "status"))
		addtimer(new Callback(src, /obj/machinery/fabricator/replicator::state_status()), 2 SECONDS, TIMER_UNIQUE | TIMER_OVERRIDE)
	else if(findtext(true_text, "menu"))
		addtimer(new Callback(src, /obj/machinery/fabricator/replicator::state_menu()), 2 SECONDS, TIMER_UNIQUE | TIMER_OVERRIDE)
	else
		for(var/datum/fabricator_recipe/recipe in SSfabrication.get_recipes(fabricator_class))
			...
			if(findtext(true_text, lowertext(recipe.name)))
				addtimer(new Callback(src, /obj/machinery/fabricator::try_queue_build(), recipe, 1), 2 SECONDS)
				break
	..()
