	...
	if (do_after(owner, 0.5 SECONDS, get_turf(owner), DO_DEFAULT | DO_USER_UNIQUE_ACT | DO_PUBLIC_PROGRESS) && owner)
		...
		for(var/turf/T in turfs)
			...
			if(istype(MS))
				MS.shields = src
				segments += MS
				GLOB.moved_event.register(MS, src, src::on_moved())

		passive_power_use = 0.8 KILOWATTS * length(segments)

		update_icon()
		owner.update_icon()
		GLOB.moved_event.register(owner, src, src::on_moved())
		GLOB.dir_set_event.register(owner, src, src::on_turned())
...
/obj/item/mech_equipment/atmos_shields/deactivate()
	for(var/obj/effect/mech_shield/MS in segments)
		if(istype(MS))
			GLOB.moved_event.unregister(MS, src, src::on_moved())
	if(length(segments))
		owner.visible_message(SPAN_WARNING("The energy shields in front of \the [owner] disappear!"))
	QDEL_NULL_LIST(segments)
	passive_power_use = 0
	GLOB.moved_event.unregister(owner, src, src::on_moved())
	GLOB.dir_set_event.unregister(owner, src, src::on_turned())
	. = ..()
	update_icon()
	owner.update_icon()
