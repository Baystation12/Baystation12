...
/obj/structure/heavy_vehicle_frame/attackby(var/obj/item/thing, var/mob/user)
	...
	if(isCrowbar(thing))
		if(is_reinforced == FRAME_REINFORCED)
			if(!do_after(user, 5 * user.skill_delay_mult(SKILL_DEVICES), src) || !material)
				return
	...
	// Installing wiring.
		...
		if(!do_after(user, 30 * user.skill_delay_mult(SKILL_ELECTRICAL), src))
			return
	...
	// Securing wiring.
		...
		if(!do_after(user, 30 * user.skill_delay_mult(SKILL_ELECTRICAL), src) || last_wiring_state != is_wired)
			return
	...
	// Installing metal.
		...
		if(M.material)
			...
			if(!do_after(user, 30 * user.skill_delay_mult(SKILL_DEVICES), src) || is_reinforced)
				return
	...
	// Welding metal.
		...
		if(WT.remove_fuel(1, user))
			...
			if(!do_after(user, 20 * user.skill_delay_mult(SKILL_DEVICES), src) || last_reinforced_state != is_reinforced)
				return
...
/obj/structure/heavy_vehicle_frame/proc/install_component(var/obj/item/thing, var/mob/user)
	...
	if(user)
		...
		if(!user.canUnEquip(thing) || !do_after(user, 30 * user.skill_delay_mult(SKILL_DEVICES), src) || user.get_active_hand() != thing)
			return
...
/obj/structure/heavy_vehicle_frame/proc/uninstall_component(var/obj/item/component, var/mob/user)
	if(!istype(component) || (component.loc != src) || !istype(user))
		return FALSE
	if(!do_after(user, 40 * user.skill_delay_mult(SKILL_DEVICES), src) || component.loc != src)
		return FALSE
