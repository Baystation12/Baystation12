...
/mob/living/exosuit/handle_hud_icons()
	...
	if(istype(C))
		...
		hud_power.maptext = SPAN_WITH_STYLE("font-family: 'Small Fonts'; -dm-text-outline: 1 black; font-size: 7px;",  "[round(get_cell().charge)]/[round(get_cell().maxcharge)]")
	else
		hud_power.maptext_x = 16
		hud_power.maptext_y = -8
		hud_power.maptext = SPAN_WITH_STYLE("font-family: 'Small Fonts'; -dm-text-outline: 1 black; font-size: 7px;", "CHECK POWER")
