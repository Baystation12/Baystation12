...
/mob/living/exosuit/ClickOn(var/atom/A, var/params, var/mob/user)
	...
	var/fail_prob = (user != src && istype(AM) && AM.loc != src) ? user.skill_fail_chance(SKILL_MECH, 30, SKILL_EXPERT, 1) : 0
	var/failed = FALSE
	if(prob(fail_prob))
	...
	if(!failed)
		if(selected_system)
			...
			if(failed)
				var/list/other_atoms = range(1, A) - A
				A = null
	...
	if(user.a_intent != I_HURT && istype(thing, /obj/item/mech_equipment))
		...
		for(var/hardpoint in hardpoints)
			if(install_system(thing, hardpoint, user))
				return
		to_chat(user, SPAN_WARNING("\The [src] has no available, compatible hardpoints to use."))
		return
