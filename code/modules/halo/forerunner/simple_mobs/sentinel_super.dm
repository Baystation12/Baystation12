...
/obj/item/projectile/beam/sentinel_super
    ...
    on_hit(atom/target)
        ...
        if(isturf(target) || ismob(target))
            ...
            if(T)
                explosion(T, -1, 0, 1, 4, 0)
                visible_message("<span class='warning'>[src] detonates in a powerful hardlight explosion!</span>")
...
/mob/living/simple_animal/hostile/sentinel/super
    name = "Super Sentinel"
    desc = "A larger, more advanced Forerunner defense drone that unleashes a single explosive hardlight bolt."
    faction = "Forerunner"
    ...
    var/obj/item/weapon/gun/energy/laser/sentinel_super_beam/sentinel_super_beam
    ...
    death(gibbed, deathmessage = "collapses in a shower of sparks!", show_dead_message = 1)
