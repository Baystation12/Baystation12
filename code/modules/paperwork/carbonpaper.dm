...
/obj/item/weapon/paper/carbon/verb/removecopy()
	...
	if (copied == 0)
		...
		copycontents = replacetext(copycontents, "<font face=\"[c.markerfont]\" color=", "<font face=\"[c.markerfont]\" nocolor=")	//This basically just breaks the existing color tag, which we need to do because the innermost tag takes priority.
