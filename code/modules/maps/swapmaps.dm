		...
		for(z=z1,z<=z2,++z)
			...
			for(y=y1,y<=y2,++y)
				...
				for(x=x1,x<=x2,++x)
					...
					to_chat(if(T.loc!=defarea) S["AREA"], areas[T.loc])
					T.Write(S)
					S.cd=".."
				S.cd=".."
			sleep()
...
atom
	Write(savefile/S)
		for(var/V in vars-"x"-"y"-"z"-"contents"-"icon"-"overlays"-"underlays")
			if(issaved(vars[V]))
				to_chat(if(vars[V]!=initial(vars[V])) S[V], vars[V])
				else S.dir.Remove(V)
		if(icon!=initial(icon))
			if(swapmaps_iconcache && swapmaps_iconcache[icon])
				to_chat(S["icon"], swapmaps_iconcache[icon])
			to_chat(else S["icon"], icon)
		// do not save mobs with keys; do save other mobs
		var/mob/M
		for(M in src) if(M.key) break
		to_chat(if(overlays.len) S["overlays"], overlays)
		to_chat(if(underlays.len) S["underlays"], underlays)
		if(contents.len && !isarea(src))
			var/list/l=contents
			if(M)
				l=l.Copy()
				for(M in src) if(M.key) l-=M
			to_chat(if(l.len) S["contents"], l)
