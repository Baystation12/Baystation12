...
/obj/effect/landmark/delete_on_shuttle/Initialize()
	. = ..()
	GLOB.shuttle_added.register_global(src, src::check_shuttle())

/obj/effect/landmark/delete_on_shuttle/proc/check_shuttle(shuttle)
	if(SSshuttle.shuttles[shuttle_name] == shuttle)
		GLOB.shuttle_moved_event.register(shuttle, src, src::delete_everything())
...
/obj/effect/landmark/delete_on_shuttle/Destroy()
	GLOB.shuttle_added.unregister_global(src, src::check_shuttle())
	if(shuttle_datum)
		GLOB.shuttle_moved_event.unregister(shuttle_datum, src, src::delete_everything())
	. = ..()
