...
/mob/living/bot/proc/startPatrol()
	...
	if(T)
		patrol_path = AStar(get_turf(loc), T, /turf::CardinalTurfsWithAccess(), /turf::Distance(), 0, max_patrol_dist, id = botcard, exclude = obstacle)
		if(!patrol_path)
			patrol_path = list()
		obstacle = null
	return
...
/mob/living/bot/proc/calcTargetPath()
	target_path = AStar(get_turf(loc), get_turf(target), /turf::CardinalTurfsWithAccess(), /turf::Distance(), 0, max_target_dist, id = botcard, exclude = obstacle)
	if(!target_path)
		if(target && target.loc)
			ignore_list |= target
		resetTarget()
