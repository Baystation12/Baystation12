/mob/living/deity
	...
	/// Minds of those who follow him
	var/list/minions = list()
	/// The objs that this dude controls.
...
/mob/living/deity/death()
	...
	if(.)
		for(var/m in minions)
			...
			to_chat(M.current, SPAN_OCCULT("Your connection has been severed! \The [src] is no more!"))
...
/mob/living/deity/proc/GodName(newname)
	SetName(form.name)
	newname = sanitize(input(src, "Choose a name for your new form.", "Name change", form.name) as text, MAX_NAME_LEN)
	if(newname)
