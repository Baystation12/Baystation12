...
/singleton/maneuver/leap/perform(mob/living/user, atom/target, strength, reflexively = FALSE)
	...
	if(.)
		...
		user.throw_at(get_turf(target), strength, 1, user, FALSE, new Callback(src, /singleton/maneuver/leap::end_leap(), user, target, old_pass_flags))
		addtimer(new Callback(user, /mob/living::jump_layer_shift_end()), 4.5)
...
/singleton/maneuver/leap/quick/perform(mob/living/user, atom/target, strength, reflexively = FALSE)
	...
	if (.)
		...
		animate(user, pixel_z = 16, time = 3, easing = SINE_EASING | EASE_IN)
		animate(pixel_z = user.default_pixel_z, time = 3, easing = SINE_EASING | EASE_OUT)
		user.throw_at(get_turf(target), strength, 1, user, FALSE, new Callback(src, /singleton/maneuver/leap::end_leap(), user, target))
		addtimer(new Callback(user, /mob/living::jump_layer_shift_end()), 4.5)
