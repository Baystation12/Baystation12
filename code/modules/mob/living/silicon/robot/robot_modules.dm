...
//	src.modules += new /obj/item/weapon/inflatable_dispenser/robot(src)	//Might as well make a new version of this that deploys weak energy barriers that act as a temporaru fix for breaches.
	...
	var/datum/matter_synth/nanolaminate = new /datum/matter_synth/nanolaminate(9999999999999)
	synths += metal
	synths += glass
	synths += plasteel
	synths += wire
	...
	var/obj/item/stack/material/cyborg/covenant/nanolaminate/ZW = new (src)
