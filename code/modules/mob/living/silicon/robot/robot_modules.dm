...
// Standard module. Does not excel in anything but can perform minor engineering/utility tasks
...
/obj/item/weapon/robot_module/robot/medical/New()
	..()
	src.modules += new /obj/item/device/healthanalyzer(src)
	...
	var/obj/item/stack/nanopaste/NP = new /obj/item/stack/nanopaste(src)
	NP.uses_charge = 1
	NP.charge_costs = list(1000)
	NP.synths = list(medicine)
	src.modules += NP

	var/obj/item/stack/medical/advanced/ointment/OI = new /obj/item/stack/medical/advanced/ointment(src)
	OI.uses_charge = 1
	OI.charge_costs = list(500)
	OI.synths = list(medicine)
	src.modules += OI

	var/obj/item/stack/medical/advanced/bruise_pack/BR = new /obj/item/stack/medical/advanced/bruise_pack(src)
	BR.uses_charge = 1
	BR.charge_costs = list(500)
	BR.synths = list(medicine)
	src.modules += BR

	var/obj/item/stack/medical/splint/SP = new /obj/item/stack/medical/splint(src)
	SP.uses_charge = 1
	SP.charge_costs = list(500)
	SP.synths = list(medicine)
	src.modules += SP

/obj/item/weapon/robot_module/robot/medical/respawn_consumable(var/mob/living/silicon/robot/R, var/amount)
	if(src.emag)
...
/obj/item/weapon/robot_module/robot/engineering/New()
	...
	var/datum/matter_synth/wire = new /datum/matter_synth/wire(50)
	var/datum/matter_synth/wood = new /datum/matter_synth/wood(20000)
	var/datum/matter_synth/plastic = new /datum/matter_synth/plastic(20000)
	var/datum/matter_synth/borosilicate = new /datum/matter_synth/borosilicate(20000)
	synths += metal
	synths += glass
	synths += plasteel
	synths += wire
	...
	var/obj/item/stack/material/cyborg/boroglass/BG = new (src)
	RG.synths = list(borosilicate)
	src.modules += BG

	var/obj/item/stack/material/cyborg/boroglass/reinforced/BRG = new (src)
	RG.synths = list(metal, borosilicate)
	src.modules += BRG



// Security module. Equipped with basic weapons and security-related tools.
...
// Janitor module. Useful for cleaning.
...
/obj/item/weapon/robot_module/robot/janitor/New()
	...
	var/datum/matter_synth/glass = new /datum/matter_synth/glass(10000)	// Light replacer easy refilling.
	synths += glass
...
// Miner module. Can operate mining drills or mine manually
