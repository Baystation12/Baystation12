...
/obj/screen/borer/toggle_host_control/Click(location, control, params)
	...
	if(!do_after(worm, 100+(worm.host.getBrainLoss()*5), do_flags = DO_DEFAULT & ~DO_BOTH_UNIQUE_ACT || !worm.host || !worm.can_use_borer_ability()))
		return
...
/obj/screen/borer/leave_host/Click(location, control, params)
	...
	if(!do_after(worm, 10 SECONDS, do_flags = DO_DEFAULT & ~DO_BOTH_UNIQUE_ACT) || !worm.can_use_borer_ability())
		return
