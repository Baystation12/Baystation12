...
/mob/living/simple_animal/hostile/retaliate/goat/use_item(var/obj/item/O as obj, var/mob/user as mob)
...
/mob/living/simple_animal/passive/cow/use_item(var/obj/item/O as obj, var/mob/user as mob)
	var/obj/item/reagent_containers/glass/G = O
	if(stat == CONSCIOUS && istype(G) && G.is_open_container())
		user.visible_message("<span class='notice'>[user] milks [src] using \the [O].</span>")
		var/transfered = udder.trans_type_to(G, /datum/reagent/drink/milk, rand(5,10))
...
/mob/living/simple_animal/passive/chicken/use_item(var/obj/item/O as obj, var/mob/user as mob)
