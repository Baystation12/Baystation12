...
/mob/living/simple_animal/friendly/opossum/adjustBruteLoss(damage)
	. = ..()
	if(damage >= 3)
		respond_to_damage()

/mob/living/simple_animal/friendly/opossum/adjustFireLoss(damage)
	. = ..()
	if(damage >= 3)
		respond_to_damage()

/mob/living/simple_animal/friendly/opossum/lay_down()
	. = ..()
	update_icon()

/mob/living/simple_animal/friendly/opossum/proc/respond_to_damage()
...
/mob/living/simple_animal/friendly/opossum/on_update_icon()
...
/mob/living/simple_animal/friendly/opossum/poppy/hear_say(var/message, var/verb = "says", var/datum/language/language = null, var/alt_name = "",var/italics = 0, var/mob/speaker = null, var/sound/speech_sound, var/sound_vol)
	. = ..()
	addtimer(CALLBACK(src, .proc/check_keywords, message), rand(1 SECOND, 3 SECONDS))

/mob/living/simple_animal/friendly/opossum/poppy/proc/check_keywords(var/message)
