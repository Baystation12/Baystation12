	...
	switch(limb)
		if(BP_HEAD)
			if(H.check_body_part_coverage(EARS))
				return 1
		if(BP_CHEST)
			if(H.check_body_part_coverage(UPPER_TORSO) && limb_covered(C, BP_R_ARM) && limb_covered(C, BP_L_ARM) && limb_covered(C, BP_R_LEG) && limb_covered(C, BP_L_LEG)) //any gap in protection will allow a borer to squeeze underneath chest protection
				return 1
		if(BP_R_ARM)
