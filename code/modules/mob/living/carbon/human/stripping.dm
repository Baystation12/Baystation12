/mob/living/carbon/human/proc/handle_strip(var/slot_to_strip_text,var/mob/living/user,var/obj/item/clothing/holder)
	...
	switch (slot_to_strip_text)
		if ("pockets")
			if (stripping)
				...
				if (do_after(user, strip_delay, src, do_flags = (DO_DEFAULT | DO_PUBLIC_PROGRESS) & ~DO_BOTH_UNIQUE_ACT))
					empty_pockets(user)
			else
	...
	if (!do_after(user, strip_delay, src, DO_DEFAULT & ~DO_BOTH_UNIQUE_ACT))
		return
