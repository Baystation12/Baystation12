	...
	switch(M.a_intent)
		if(I_HELP)
			...
			if(H != src && istype(H) && (is_asystole() || (status_flags & FAKEDEATH) || failed_last_breath) && !(H.zone_sel.selecting == BP_R_ARM || H.zone_sel.selecting == BP_L_ARM))
				...
				if(!do_after(H, cpr_delay, src, DO_DEFAULT & ~DO_BOTH_UNIQUE_ACT))
					cpr_time = 1
					return
				cpr_time = 1
...
/mob/living/carbon/human/proc/apply_pressure(mob/living/user, var/target_zone)
	...
	spawn(0)
		...
		do_after(user, INFINITY, src, do_flags = (DO_DEFAULT & ~DO_SHOW_PROGRESS & ~DO_TARGET_UNIQUE_ACT) | DO_USER_SAME_ZONE)
