...
/mob/living/carbon/human/welding_eyecheck()
	...
	switch(safety)
		if(FLASH_PROTECTION_MODERATE)
			...
			E.take_general_damage(rand(1, 2), damage_type = DAMAGE_BURN)
			if (E.get_damage_value() > 12)
				eye_blurry += rand(3,6)
		if(FLASH_PROTECTION_MINOR)
			to_chat(src, "<span class='warning'>Your eyes stings!</span>")
			E.take_general_damage(rand(1, 4), damage_type = DAMAGE_BURN)
			if (E.get_damage_value() > 10)
				eye_blurry += rand(3,6)
				E.take_general_damage(rand(1, 4), damage_type = DAMAGE_BURN)
		if(FLASH_PROTECTION_NONE)
			to_chat(src, "<span class='warning'>Your eyes burn!</span>")
			E.take_general_damage(rand(2, 4), damage_type = DAMAGE_BURN)
			if (E.get_damage_value() > 10)
				E.take_general_damage(rand(4, 10), damage_type = DAMAGE_BURN)
		if(FLASH_PROTECTION_REDUCED)
			...
			E.take_general_damage(rand(12, 16), damage_type = DAMAGE_BURN)
