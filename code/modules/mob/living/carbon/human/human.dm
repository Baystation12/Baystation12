	...
	//			var/atom/target = get_edge_target_turf(user, get_dir(src, get_step_away(user, src)))
		...
		if (2.0)
			...
			if (!istype(l_ear, /obj/item/clothing/ears/earmuffs) && !istype(r_ear, /obj/item/clothing/ears/earmuffs))
				ear_damage = min(ear_damage + 30,100*species.explosion_affect_mod)
				ear_deaf = min(ear_damage + 120,200*species.explosion_affect_mod)
			if (prob(70))
				confused = min(confused + 10,50*species.explosion_affect_mod)

		if(3.0)
			b_loss = 30
			if (!istype(l_ear, /obj/item/clothing/ears/earmuffs) && !istype(r_ear, /obj/item/clothing/ears/earmuffs))
				ear_damage = min(ear_damage + 15,100*species.explosion_affect_mod)
				ear_deaf = min(ear_damage + 60,200*species.explosion_affect_mod)
			if (prob(50))
				confused = min(confused + 10,50*species.explosion_affect_mod)
