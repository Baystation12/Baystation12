...
/mob/living/carbon/human/proc/rip_out_organ()
	set category = "Object"
	set name = "Rip out organ"
	set desc = "Rip out their organs! The organ probably won't survive this."
	set src in view(1)

	var/mob/user = usr
	var/list/exposed_internal_organs = list()
	var/alist/organ_parent_map = alist() // because internal organs have no knowledge whatsoever of their owner
	var/obj/item/organ/internal/organ
	var/obj/item/organ/external/parent_organ

	for (var/obj/item/organ/external/limb in organs)
		if (!(limb.encased && HAS_FLAGS(limb.status, ORGAN_BROKEN) && limb.how_open() >= SURGERY_OPEN))
			continue
		else if (limb.how_open() < SURGERY_OPEN)
			continue
		for (var/obj/item/organ/internal_organ as anything in limb.internal_organs)
			if (istype(internal_organ, /obj/item/organ/internal/brain))
				continue
			var/image/radial_button = image(icon = internal_organ.icon, icon_state = internal_organ.icon_state)
			radial_button.name = "Rip out \the [internal_organ.name]!"
			exposed_internal_organs[internal_organ] = radial_button
			organ_parent_map[internal_organ] = limb

	if (!length(exposed_internal_organs))
		to_chat(user, SPAN_WARNING("You can't find any organs to tear out."))
		return

	if (length(exposed_internal_organs) == 1 && user.get_preference_value(/datum/client_preference/surgery_skip_radial))
		organ = exposed_internal_organs[1]
	organ = show_radial_menu(user, src, exposed_internal_organs, radius = 42, require_near = TRUE, use_labels = TRUE)
	parent_organ = organ_parent_map[organ]

	if (!istype(organ) || !istype(parent_organ))
		return

	if (!BP_IS_ROBOTIC(organ))
		playsound(organ.loc, 'sound/effects/squelch1.ogg', 15, 1)
	else
		playsound(organ.loc, 'sound/items/Ratchet.ogg', 50, 1)

	user.visible_message(
		SPAN_DANGER("\The [user] begins to reach inside of \the [src]'s [parent_organ.name]!"),
		SPAN_DANGER("You begin to rip out \the [organ] out of \the [src]'s [parent_organ.name]!")
	)

	// final sanity checks for ripping out the organ
	if (!(user.do_skilled(3 SECONDS, SKILL_ANATOMY, src, 0.1) && \
		user.use_sanity_check(organ.owner) && \
		(organ in src) && \
		(parent_organ.encased && HAS_FLAGS(parent_organ.status, ORGAN_BROKEN) && parent_organ.how_open() >= SURGERY_OPEN) || parent_organ.how_open() >= SURGERY_OPEN))
		return
	...
	organ.add_fingerprint(user)
	parent_organ.take_external_damage(30 - (user.get_skill_value(SKILL_ANATOMY) / (SKILL_MAX - SKILL_MIN) * 20), used_weapon = "bare hands")
	custom_pain("Someone's ripping out something from inside of you! God, it hurts!", 100, affecting = parent_organ)
	organ.cut_away(user)

	if (istype(organ, /obj/item/organ/internal/mmi_holder))
		var/obj/item/organ/internal/mmi_holder/brain = organ
		brain.transfer_and_delete()
	user.put_in_hands(organ)
	parent_organ.implants -= organ
	if (istype(organ, /obj/item/organ/internal/brain))
		organ.take_internal_damage(30 - (user.get_skill_value(SKILL_ANATOMY) / (SKILL_MAX - SKILL_MIN) * 20), TRUE)
	else
		if (user.get_skill_value(SKILL_ANATOMY) == SKILL_MAX && prob(80))
			organ.take_internal_damage(organ.max_damage * 0.8, TRUE)
		else
			organ.die()
	user.visible_message(
		SPAN_DANGER("\The [user] rips out \the [organ] out of \the [src]'s [parent_organ.name]!"),
		SPAN_DANGER("You rip out \the [organ] out of \the [src]'s [parent_organ.name]!")
	)

/mob/living/carbon/human/proc/handle_internal_organ_verbs()
	for (var/obj/item/organ/external/limb in organs)
		if (limb.encased && HAS_FLAGS(limb.status, ORGAN_BROKEN) && limb.how_open() >= SURGERY_OPEN)
			verbs += /mob/living/carbon/human/proc/rip_out_organ
			return
		else if (!limb.encased && limb.how_open() >= SURGERY_OPEN)
			verbs += /mob/living/carbon/human/proc/rip_out_organ
			return
	verbs -= /mob/living/carbon/human/proc/rip_out_organ
