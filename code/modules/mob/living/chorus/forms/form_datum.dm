...
/datum/chorus_form/proc/get_buildings_by_tier()
	. = list()
	var/i = 1
	var/list/all_buildings = buildings.Copy()
	while(all_buildings.len)
		var/list/cur_tier = list()
		for(var/b in all_buildings)
			var/datum/chorus_building/cb = all_buildings[b]
			if(cb.build_level > i)
				continue
			all_buildings -= b
			cur_tier += cb
