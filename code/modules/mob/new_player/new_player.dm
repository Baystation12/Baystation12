...
/mob/new_player/Topic(href, href_list[])
	...
	if(href_list["observe"])
		...
		if(!config.respawn_delay || client.holder || alert(src,"Are you sure you wish to observe? You will have to wait [config.respawn_delay * 2] minute(s) before being able to respawn!","Player Setup","Yes","No") == "Yes")
			if(!client)	return 1
