...
/mob/living/carbon/human/verb/tie_hair()
    ...
    if(!(src.species.name in list("Human","Vat-Grown Human","Space-Adapted Human","Grav-Adapted Human")) )
        to_chat(src, "<span class='notice'>Only humans can use this verb.</span>")
        return

    if(src.restrained())
        to_chat(src, "<span class='warning'>You are restrained and cannot do that.</span>")
        return

    var/obj/item/organ/external/head = src.get_organ(BP_HEAD)

    if(head.owner.h_style)
        var/style = head.owner.h_style
        var/datum/sprite_accessory/hair/hair_style = hair_styles_list[style]
        var/selected_string
        if(!(hair_style.flags & HAIR_TIEABLE))
            to_chat(src, "<span class ='warning'>Your hair is not the right length to do this!</span>")
        else
            var/list/datum/sprite_accessory/hair/valid_hairstyles = list()
            for(var/temp_style in hair_styles_list)
            ...
            selected_string = input("Select a new hairstyle", "Your hairstyle", hair_style) in valid_hairstyles
