datum/preferences
	...
	proc/update_preview_icon()		//seriously. This is horrendous.
			...
			if(job_civilian_low & ASSISTANT)//This gives the preview icon clothes depending on which job(if any) is set to 'high'
				...
				switch(job_medsci_high)
					...
					if(DOCTOR)
						...
						clothes_s.Blend(new /icon('icons/mob/feet.dmi', "white"), ICON_UNDERLAY)
						clothes_s.Blend(new /icon('icons/mob/suit.dmi', "labcoat_open"), ICON_OVERLAY)
						clothes_s.Blend(new /icon(INV_R_HAND_DEF_ICON, "firstaid-advanced"), ICON_UNDERLAY)
						switch(backbag)
							if(2)
								clothes_s.Blend(new /icon('icons/mob/back.dmi', "medicalpack"), ICON_OVERLAY)
							if(3)
					...
					if(PARAMEDIC)
						...
						clothes_s.Blend(new /icon('icons/mob/belt.dmi', "emsbelt"), ICON_OVERLAY)
				...
				switch(job_engsec_high)
					...
					if(DETECTIVE)
						...
						clothes_s.Blend(new /icon(INV_R_HAND_DEF_ICON, "case"), ICON_UNDERLAY)
						switch(backbag)
							if(2)
								clothes_s.Blend(new /icon('icons/mob/back.dmi', "backpack"), ICON_OVERLAY)
							if(3)
			...
			if(current_species.appearance_flags & HAS_UNDERWEAR)
				for(var/underwear_category_name in all_underwear)
					...
					if(underwear_category)
						...
						overlays += underwear_item.generate_image(all_underwear_metadata[underwear_category_name])
					else
		...
		O.icon = preview_icon
		O.overlays = overlays
		preview_icon_front = getFlatIcon(O, defdir = SOUTH, always_use_defdir = TRUE)
		preview_icon_side = getFlatIcon(O, defdir = WEST, always_use_defdir = TRUE)
		qdel(O)
