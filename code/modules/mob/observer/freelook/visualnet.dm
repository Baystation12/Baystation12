...
/datum/visualnet/proc/major_chunk_change(atom/source)
	for_all_chunks_in_range(source, /datum/chunk::visibility_changed(), list())
...
/datum/visualnet/proc/add_source(atom/source, update_visibility = TRUE, opacity_check = FALSE)
	...
	if(source in sources)
		return FALSE
	sources += source
	GLOB.moved_event.register(source, src, /datum/visualnet::source_moved())
	GLOB.destroyed_event.register(source, src, /datum/visualnet::remove_source())
	for_all_chunks_in_range(source, /datum/chunk::add_source(), list(source))
...
/datum/visualnet/proc/remove_source(atom/source, update_visibility = TRUE, opacity_check = FALSE)
	...
	GLOB.moved_event.unregister(source, src, /datum/visualnet::source_moved())
	GLOB.destroyed_event.unregister(source, src, /datum/visualnet::remove_source())
	for_all_chunks_in_range(source, /datum/chunk::remove_source(), list(source))
	if(update_visibility)
		update_visibility(source, opacity_check)
	return TRUE
...
/datum/visualnet/proc/source_moved(atom/movable/source, old_loc, new_loc)
	...
	if(old_turf)
		for_all_chunks_in_range(source, /datum/chunk::remove_source(), list(source), old_turf)
	if(new_turf)
		for_all_chunks_in_range(source, /datum/chunk::add_source(), list(source), new_turf)
