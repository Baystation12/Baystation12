...
/datum/skills_repository/New()
	...
	for (var/singleton/skill_category/category as anything in categories)
		...
		for (var/singleton/skill/skill as anything in category.skills)
			skills += skill
...
/singleton/skill_category/service
	name = "Service"
	skills = list(
...
/singleton/skill
	...
	/// A unique string used to identify this skill in save data.
	var/key
	...
	var/const/LEVEL_NONE = "Unskilled"
	var/const/LEVEL_BASIC = "Basic"
	var/const/LEVEL_ADEPT = "Trained"
	var/const/LEVEL_EXPERT = "Experienced"
	var/const/LEVEL_PROF = "Master"

	/// A (level name = level description) list for different levels of this skill.
	var/list/levels
...
/singleton/skill/New()
	category = GET_SINGLETON(category)
...
/singleton/skill/combat/get_cost(level)
	switch (level)
		if (SKILL_BASIC)
			return difficulty
...
/singleton/skill/weapons/get_cost(level)
	switch (level)
		if (SKILL_BASIC)
			return difficulty
		if (SKILL_ADEPT)
...
/singleton/skill/forensics/get_cost(level)
	switch (level)
