...
/datum/material/steel/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)	//recipes below don't support composite materials
		return
...
/datum/material/plasteel/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)	//recipes below don't support composite materials
		return
...
/datum/material/stone/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)	//recipes below don't support composite materials
		return
	. += new/datum/stack_recipe/furniture/planting_bed(src)

/datum/material/plastic/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)	//recipes below don't support composite materials
		return
...
/datum/material/wood/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)	//recipes below don't support composite materials
		return
...
/datum/material/wood/mahogany/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)
		return
	. += new/datum/stack_recipe/tile/mahogany(src)

/datum/material/wood/maple/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)
		return
	. += new/datum/stack_recipe/tile/maple(src)

/datum/material/wood/ebony/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)
		return
	. += new/datum/stack_recipe/tile/ebony(src)

/datum/material/wood/walnut/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)
		return
	. += new/datum/stack_recipe/tile/walnut(src)

/datum/material/cardboard/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)	//recipes below don't support composite materials
		return
...
/datum/material/aluminium/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)	//recipes below don't support composite materials
		return
	. += new/datum/stack_recipe/grenade(src)

/datum/material/leather/generate_recipes(reinforce_material)
	. = ..()
	if(reinforce_material)	//recipes below don't support composite materials
		return
