...
/datum/TCS_Compiler
	...
	proc/Run(var/datum/signal/signal)
		...
		*/
		...
		/* sanitize_russian EVERYTHING. fucking players can't be trusted with SHIT */
...
datum/signal
	...
	proc/tcombroadcast(var/message, var/freq, var/source, var/job)
		...
		if((!message || message == "") && message != 0)
			message = "*beep*"
		if(!source)
			source = "[html_encode_russian(uppertext(S.id))]"
		...
		if(source in S.stored_names)
			...
			newsign.data["name"] = "<i>[html_encode_russian(uppertext(source))]<i>"
