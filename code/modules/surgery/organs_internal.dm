// Consider adding: "nutrient vessel","receptor node","egg sac","plasma vessel","acid gland","hive node","resin spinner"
var/list/transplantable_special_organs = list("stack") // This should go somewhere else.
...
/datum/surgery_step/internal/replace_organ
	...
	can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		...
		if(O.organ_tag == "limb")
			return 0
		else if((O.organ_tag in transplantable_special_organs) || target.species.has_organ[O.organ_tag])

			if(O.damage > (O.max_damage * 0.75))
				user << "<span class='danger'>\The [O.organ_tag] [o_is] in no state to be transplanted.</span>"
				return 2

			if(!target.internal_organs_by_name[O.organ_tag])
				organ_missing = 1
			else
				user << "<span class='danger'>\The [target] already has [o_a][O.organ_tag].</span>"
				return 2

			if(O && affected.limb_name == O.parent_organ)
				organ_compatible = 1
			else
				user << "<span class='danger'>\The [O.organ_tag] [o_do] normally go in \the [affected.name].</span>"
				return 2
		else
			user << "<span class='danger'>You're pretty sure [target.species.name_plural] don't normally have [o_a][O.organ_tag].</span>"
			return 2
