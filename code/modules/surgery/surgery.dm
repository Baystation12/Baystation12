	...
	// Otherwise we can make a start on surgery!
		...
		if(zone in M.surgeries_in_progress)
			...
			if(operation_data)
				...
				if(prob(S.success_chance(user, M, src, zone)) && do_after(user, duration, M, DO_DEFAULT | DO_PUBLIC_PROGRESS, do_skill = skill_reqs[1], delay_flags = DO_AFTER_TIME_FLAG_USER_SKILL))
