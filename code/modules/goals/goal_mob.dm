...
/mob/proc/show_goals(show_success = FALSE, allow_modification = FALSE)
	...
	//No goals to display
	if(!(allow_modification || length(mind.goals)) && !(dept && length(dept.goals)))
		return
	...
	if(allow_modification && !prefs_no_personal_goals && length(mind.goals) < max_goals)
