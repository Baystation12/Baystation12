...
/mob/proc/show_goals(var/show_success = FALSE, var/allow_modification = FALSE)
	...
	if(dept && get_preference_value(/datum/client_preference/show_department_goals) == GLOB.PREF_SHOW)
		if(LAZYLEN(dept.goals))
			to_chat(src, SPAN_NOTICE("<br><br><font size = 3><b>В этой смене, [dept.name] преследует следующие ведомственные цели:</b></font><br>[jointext(dept.summarize_goals(show_success), "<br>")]"))
		else
			to_chat(src, SPAN_NOTICE("<br><br><font size = 3><b>[dept.name] не имеет ведомственных целей в этой смене.</b></font>"))
