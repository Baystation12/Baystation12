...
/datum/goal/proc/summarize(show_success = FALSE, allow_modification = FALSE, mob/user)
	. = "[description][get_summary_value()]"
	if(show_success)
		. += get_success_string()
	if(allow_modification)
		if(can_abandon) . += " (<a href='byond://?src=\ref[owner];abandon_goal=\ref[src];abandon_goal_caller=\ref[user]'>Abandon</a>)"
		if(can_reroll)  . += " (<a href='byond://?src=\ref[owner];reroll_goal=\ref[src];reroll_goal_caller=\ref[user]'>Reroll</a>)"
