...
/datum/goal/proc/summarize(var/show_success = FALSE, var/allow_modification = FALSE, var/position = 1)
	. = "[description][get_summary_value()]"
	if(show_success)
		. += get_success_string()
	if(allow_modification)
		if(can_abandon) . += " (<a href='?src=\ref[owner];abandon_goal=[position]'>Abandon</a>)"
		if(can_reroll)  . += " (<a href='?src=\ref[owner];reroll_goal=[position]'>Reroll</a>)"
