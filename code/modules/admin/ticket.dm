...
/datum/ticket/New(datum/client_lite/owner)
	...
	addtimer(new Callback(src, src::timeoutcheck()), 5 MINUTES, TIMER_STOPPABLE)
...
/datum/ticket/proc/timeoutcheck()
	if(status == TICKET_OPEN)
		...
		addtimer(new Callback(src, src::timeoutchecklast()), 5 MINUTES, TIMER_STOPPABLE)
...
/datum/ticket/proc/timeoutchecktaken()
	if (status == TICKET_ASSIGNED)
		if (is_active())
			addtimer(new Callback(src, src::timeoutchecktaken()), 30 MINUTES, TIMER_STOPPABLE)
			return
