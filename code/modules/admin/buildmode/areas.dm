...
/datum/build_mode/areas/Configurate()
	...
	if (mode == "Pick")
		...
		if (!new_name)
			return
		selected_area = new
		selected_area.SetName(new_name)
		...
		to_chat(user, "Created area [selected_area.name]")
...
/area/Destroy()
	for (var/client/C in world)
		if (C.mob && C.mob.click_handlers)
			var/CH = C.mob.click_handlers[1]
			if (istype(CH, /datum/click_handler/build_mode))
				for (var/datum/build_mode/BM in CH)
					if (istype(BM, /datum/build_mode/areas))
						var/datum/build_mode/areas/BMA = BM
						if (BMA.selected_area == src)
							to_chat(BMA.user, "Area [src.name] was destroyed.")
							BMA.selected_area = null
						break
	. = ..()
