...
/datum/callproc/proc/finalise()
	...
	if(hastarget)
		if(!target)
			to_chat(usr, "Your callproc target no longer exists.")
			return
		log_and_message_admins("[key_name(C)] called \the [target]'s [procname]() with [length(arguments) ? "the arguments [list2params(arguments)]" : "no arguments"].", location = get_turf(target))
		if(length(arguments))
			returnval = call(target, procname)(arglist(arguments))
		else
			returnval = call(target, procname)()
	else
		log_and_message_admins("[key_name(C)] called [procname]() with [length(arguments)? "the arguments [list2params(arguments)]" : "no arguments"].", location = get_turf(target))
