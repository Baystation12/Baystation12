...
/mob/living/carbon/human/proc/equip_raider()
	...
	C.name = "[real_name]'s Passport"
	C.icon_state = "id"
	C.access = list(access_syndicate)
	C.assignment = "Visitor"
...
/mob/living/carbon/human/proc/equip_vox_raider(var/include_armour)
	...
	if(include_armour)
		...
		switch(vox_tick)
			if(1) // Vox raider!
				...
				equip_to_slot_or_del(new /obj/item/weapon/gun/launcher/spikethrower(src), slot_r_hand)
			if(2) // Vox engineer!
	...
	equip_to_slot_or_del(new /obj/item/clothing/mask/gas/swat/vox(src), slot_wear_mask)
	...
	if(istype(internal,/obj/item/weapon/tank) && internals)
		...
		world << "What the fuck is wrong with your internals."
