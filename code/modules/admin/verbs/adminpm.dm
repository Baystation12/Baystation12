...
/client/proc/cmd_admin_pm(var/client/C, var/msg = null)
	...
	if(!istype(C,/client))
			to_chat(if(holder)	src, "<span class='warning'>Error: Private-Message: Client not found.</span>")
				to_chat(else		src, "<span class='warning'>Error: Private-Message: Client not found. They may have lost connection, so try using an adminhelp!</span>")
		return
	...
	if(!msg)
		...
		if(!msg)	return
		if(!C)
				to_chat(if(holder)	src, "<span class='warning'>Error: Admin-PM: Client not found.</span>")
					to_chat(else		src, "<span class='warning'>Error: Private-Message: Client not found. They may have lost connection, so try using an adminhelp!</span>")
			return
