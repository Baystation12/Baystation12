/client/proc/cmd_admin_say(msg as text)
	...
	/*for (var/client/C in admin_list)
		if (src.holder.rank == "Admin Observer")
			...
			C << "<span class='admin'><span class='prefix'>ADMIN:</span> <EM>[key_name(usr, C)]</EM> (<A HREF='?src=\ref[C.holder];adminplayerobservejump=\ref[mob]'>JMP</A>): <span class='message'>[msg]</span></span>"/client/proc/cmd_mod_say(msg as text)*/
...
/client/proc/cmd_mod_say(msg as text)
	...
	if (!src.holder)
		...
		return	//todo: what? why does this not compile
	/*if (src.muted || src.muted_complete)		src << "You are muted."
		return*/
	msg = copytext(sanitize(msg), 1, MAX_MESSAGE_LEN)
