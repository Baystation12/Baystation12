...
/datum/admin_secret_item/random_event/gravity/execute(var/mob/user)
	...
	message_admins("<span class='notice'>[key_name_admin(user)] toggled gravity [gravity_is_on ? "on" : "off"].</span>", 1)

	if(choice == "Yes")
		if(gravity_is_on)
			command_announcement.Announce("Gravity generator are again functioning within normal parameters. Sorry for any inconvenience.", "Gravity Restored")
		else
