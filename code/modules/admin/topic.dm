...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
		...
		if(!check_rights(R_MENTOR|R_MOD|R_ADMIN|R_AUDITOR))	return
		...
		if(!check_rights(R_ADMIN|R_MOD|R_MENTOR|R_AUDITOR))	return
		...
		src.owner << "You sent [input] to [H] via a secure channel."
		log_admin_single("[src.owner] replied to [key_name(H)]'siIllegal message with the message [input].")
...
*/
	...
	if(href_list["vipeditrights"])
		if(!check_rights(R_ADMIN))
			message_admins("[key_name_admin(usr)] attempted to edit the event permissions without sufficient rights.")
			log_admin("[key_name(usr)] attempted to edit the event permissions without sufficient rights.")
			return
		...
		if(task == "add")
			var/new_ckey = ckey(input(usr,"New Event Personnel ckey","Event Personnel ckey", null) as text|null)
			if(!new_ckey)	return
		...
		if(task == "remove")
			if(alert("Are you sure you want to remove [vip_ckey]?","Message","Yes","Cancel") == "Yes")
				...
				message_admins("[key_name_admin(usr)] removed [vip_ckey] from the Event list")
				log_admin("[key_name(usr)] removed [vip_ckey] from the Event list")
			...
			if(vip_ranks.len)
				...
				new_rank = input("Please select a rank", "New rank", null, null) as null|anything in list("EventApproved", "*New Rank*")
