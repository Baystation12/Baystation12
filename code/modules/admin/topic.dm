...
//Regular jobs
	...
	// call dibs on IC messages (prays, emergency comms, faxes)
		...
		if(ticket.status == TICKET_ASSIGNED && (ticket.assigned_admin == client_repository.get_lite_client(usr) || alert(usr.client, "This ticket is already assigned to [ticket.assigned_admin.key_name(0)]. Are you sure you want to take over?",  "Take over?" , "Yes" , "No") != "Yes"))
			return
