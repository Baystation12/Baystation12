...
*/
	...
	else if(href_list["tdome"])
		if(!check_rights(R_FUN))	return

		if(alert(usr, "Confirm?", "Message", "Yes", "No") != "Yes")
			return

		var/mob/M = locate(href_list["tdome"])
		if(!ismob(M))
			usr << "This can only be used on instances of type /mob"
			return
		if(istype(M, /mob/living/silicon/ai))
		...
		message_admins("[key_name_admin(usr)] has sent [key_name_admin(M)] to the thunderdome.", 1)
...
//knock yourself out to stop the ghosts
			...
			if("shockwave")
				if(usr.client.angry == 0)
					return
				else
					switch(alert("Are you sure you want to do this? Back off now if you don't know what this does.","BACK OUT NOW","Yes","No"))
						if("No")
							return

				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","AB")
				log_admin("[usr] has activated shockwave")
				message_admins("[usr] is exercising their right to adminbus.")
			...
			if("schoolgirl")
				...
				command_announcement.Announce("TRANSLATION: Wow, this is just like one of my japanese animes!", new_sound = 'sound/AI/animes.ogg')
			if("paintball")
				feedback_inc("admin_secrets_fun_used",1)
				feedback_add_details("admin_secrets_fun_used","PB")
			...
			if("ionstorm")
				...
				if(show_log == "Yes")
					command_announcement.Announce("Ion storm detected in station proximity. Please check all AI-controlled equipment for coooorr*pt1@n!%-BZZT.", "Anomaly Alert", new_sound = 'sound/AI/ionstorm.ogg')
			if("spacevines")
				feedback_inc("admin_secrets_fun_used",1)
