	...
	//JOBBAN'S INNARDS
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				SSwebhooks.send(WEBHOOK_BAN, list("admin" = user.client.ckey, "player" = mob_key, "time" = mins_readable, "reason" = reason))
				qdel(M.client)
			...
			if("No")
				...
				SSwebhooks.send(WEBHOOK_BAN, list("admin" = user.client.ckey, "player" = mob_key, "time" = "perma", "reason" = reason))
				qdel(M.client)
				//qdel(M)
			if("Cancel")
				return
