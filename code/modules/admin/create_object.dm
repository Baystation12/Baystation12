...
/datum/admins/proc/create_object(mob/user)
	...
	user << browse(replacetext(create_object_html, "/* ref src */", "\ref[src]"), "window=create_object;size=450x500")
...
/datum/admins/proc/quick_create_object(mob/user)
	...
	user << browse(replacetext(html_form, "/* ref src */", "\ref[src]"), "window=qco[path];size=450x500")