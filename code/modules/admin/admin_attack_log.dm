...
proc/log_and_message_admins(var/message as text, var/mob/user = usr, var/turf/location)
	...
	if(T)
		message = message + " (<a HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[T.x];Y=[T.y];Z=[T.z]'>LOC</a>)"
	if(user)
		message = message + " (<a HREF='?_src_=holder;adminplayerobservefollow=\ref[user]'>MOB</a>)"
