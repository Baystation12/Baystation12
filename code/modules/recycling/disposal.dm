...
/obj/machinery/disposal/attackby(obj/item/I, mob/user)
	if(stat & MACHINE_STAT_BROKEN || !I || !user)
		return
...
/obj/machinery/disposal/MouseDrop_T(atom/movable/AM, mob/user)
	...
	if(stat & MACHINE_STAT_BROKEN || !CanMouseDrop(AM, user, incapacitation_flags) || AM.anchored || !isturf(user.loc))
		return
	...
	// Repeat checks
	if(stat & MACHINE_STAT_BROKEN || user.incapacitated(incapacitation_flags))
		return
...
/obj/machinery/disposal/Process()
	if(!air_contents || (stat & MACHINE_STAT_BROKEN))			// nothing can happen if broken
