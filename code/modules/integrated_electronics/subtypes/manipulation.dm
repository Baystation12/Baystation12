...
/obj/item/integrated_circuit/manipulation/grenade/do_work()
	if(attached_grenade && !attached_grenade.active)
		...
		if(isnum(detonation_time.data) && detonation_time.data > 0)
			...
			dt = 15
		addtimer(new Callback(attached_grenade, attached_grenade::activate()), dt)
		var/atom/holder = loc
		...
		if(1)
			if(can_pull(to_pull))
				if(check_target(to_pull, exclude_contents = TRUE))
					...
					GLOB.moved_event.register(to_pull, src, src::check_pull()) //Whenever the target moves, make sure we can still pull it!
					GLOB.destroyed_event.register(to_pull, src, src::stop_pulling()) //Stop pulling if it gets destroyed
					GLOB.moved_event.register(acting_object, src, src::pull()) //Make sure we actually pull it.
			push_data()
		if(3)
			if(pulling)
				stop_pulling()
