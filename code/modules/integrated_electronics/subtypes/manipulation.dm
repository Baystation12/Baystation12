...
/obj/item/integrated_circuit/manipulation/interactor
	name = "interactor"
	desc = "Contains a slot for an item that can be used to interact with a given target."
	icon_state = "interactor"

	inputs = list(
		"target" = IC_PINTYPE_REF
	)
	outputs = list(
		"reference to item" = IC_PINTYPE_REF
	)
	activators = list(
...
/obj/item/integrated_circuit/manipulation/interactor/Destroy()
	QDEL_NULL(installed_item)
	return ..()


/obj/item/integrated_circuit/manipulation/interactor/attackby(var/obj/item/I, var/mob/user)
	if(!istype(I))
		return ..()

	if(installed_item)
		to_chat(user, "<span class='warning'>There's already an item installed.</span>")
		return
	...
	installed_item = I
	to_chat(user, "<span class='notice'>You slide \the [I] into the interactor.</span>")
	playsound(src, 'sound/items/Crowbar.ogg', 50, 1)

	set_pin_data(IC_OUTPUT, 1, weakref(installed_item))
	push_data()
...
/obj/item/integrated_circuit/manipulation/interactor/attack_self(var/mob/user)
	if(installed_item)
		...
		to_chat(user, "<span class='notice'>There's no item to remove from the interactor.</span>")

/obj/item/integrated_circuit/manipulation/interactor/do_work()
	...
	target.attackby(installed_item)
	activate_pin(2)
