	...
	if(useMS)
		if(prob(5))
			...
			switch(pick(1,2,3,4,5,6,7))
				...
				if(2)
					sender = pick(300;"QuickDatingSystem",200;"Find your Terran bride",50;"Tajaran beauties are waiting",50;"Find your secret skrell crush",50;"Beautiful unathi brides")
					message = pick("Your profile caught my attention and I wanted to write and say hello (QuickDating).",\
					"If you will write to me on my email [pick(first_names_female)]@[pick(last_names)].[pick("tc","ck","tj","ur","nt")] I shall necessarily send you a photo (QuickDating).",\
