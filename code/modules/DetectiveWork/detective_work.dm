...
//This holds objects (1) without prints, and their fibers(2) and blood(3).
	...
	proc/add_data_scanner(var/obj/item/device/W)
		if(istype(W, /obj/item/device/detective_scanner))
			var/obj/item/device/detective_scanner/S = W
			if(S.stored)
				for(var/atom in S.stored)
					var/list/data = S.stored[atom]
			...
			if(P.cartridge.stored_data)
				for(var/atom in P.cartridge.stored_data)
					var/list/data = P.cartridge.stored_data[atom]
					add_data_master(atom,data[1],data[2],data[3],data[4])
			P.cartridge.stored_data = list()
		return
