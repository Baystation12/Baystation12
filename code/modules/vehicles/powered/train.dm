/obj/vehicle/powered/train
	...
	var/obj/vehicle/powered/train/lead
	var/obj/vehicle/powered/train/tow


//-------------------------------------------
// Standard procs
//-------------------------------------------
/obj/vehicle/powered/train/Initialize()
	. = ..()
	for(var/obj/vehicle/powered/train/T in orange(1, src))
		latch(T)

/obj/vehicle/powered/train/Move()
...
/obj/vehicle/powered/train/Bump(atom/Obstacle)
...
// Vehicle procs
//-------------------------------------------
/obj/vehicle/powered/train/explode()
	if (tow)
		tow.unattach()
	unattach()
	..()
...
// Interaction procs
//-------------------------------------------
/obj/vehicle/powered/train/relaymove(mob/user, direction)
...
/obj/vehicle/powered/train/MouseDrop_T(var/atom/movable/C, mob/user as mob)
	if(!CanPhysicallyInteract(user) || !user.Adjacent(C) || !istype(C) || (user == C))
		return
	if(istype(C,/obj/vehicle/powered/train))
		latch(C, user)
	else
		if(!load(C))
			to_chat(user, "<span class='warning'>You were unable to load [C] on [src].</span>")

/obj/vehicle/powered/train/attack_hand(mob/user as mob)
...
/obj/vehicle/powered/train/verb/unlatch_v()
	set name = "Unlatch"
...
/obj/vehicle/powered/train/proc/attach_to(obj/vehicle/powered/train/T, mob/user)
	if (get_dist(src, T) > 1)
	...
	//check for cycles.
	var/obj/vehicle/powered/train/next_car = T
	while (next_car)
		if (next_car == src)
...
/obj/vehicle/powered/train/proc/unattach(mob/user)
	if (!lead)
...
/obj/vehicle/powered/train/proc/latch(obj/vehicle/powered/train/T, mob/user)
...
/obj/vehicle/powered/train/proc/is_train_head()
	if (lead)
		return 0
	return 1
...
//-------------------------------------------------------
/obj/vehicle/powered/train/update_stats()
	//first, seek to the end of the train
	var/obj/vehicle/powered/train/T = src
	while(T.tow)
...
/obj/vehicle/powered/train/proc/update_car(var/train_length, var/active_engines)
	return
