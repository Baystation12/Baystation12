/datum/codex_category/gases/Initialize()
	for(var/gas in gas_data.gases)
		...
		if(gas_data.flags[gas] & XGM_GAS_FUSION_FUEL)
			gas_info+= "It can be used to fuel fusion reaction"
		...
		var/datum/codex_entry/entry = new(_display_name = "[gas_data.name[gas]] (gas)", _mechanics_text = jointext(gas_info, "<br>"))
