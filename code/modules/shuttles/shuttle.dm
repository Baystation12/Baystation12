...
/datum/shuttle
	...
	//The shadow effects placed where a shuttle will land.
	var/shadows = list()
...
/datum/shuttle/proc/create_shadows(obj/effect/shuttle_landmark/destination)
	...
	for(var/turf/T in translation)
		...
		//add a shadow to where the corresponding shuttle turf will be at its destination
		LAZYADD(shadows, new /obj/effect/shadow(translation[T]))
