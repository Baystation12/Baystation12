...
/datum/shuttle/autodock/ferry/emergency/shuttle_moved()
	if(next_location != waypoint_station)
		...
		priority_announcement.Announce(replacetext_char(replacetext_char((emergency_controller.emergency_evacuation ? GLOB.using_map.emergency_shuttle_leaving_dock : GLOB.using_map.shuttle_leaving_dock), "%dock_name%", "[GLOB.using_map.dock_name]"),  "%ETA%", "[round(emergency_controller.get_eta()/60,1)] minute\s"))
