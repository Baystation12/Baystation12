...
/datum/organ/external/proc/droplimb(var/override = 0,var/no_explode = 0,var/showmessage = 1,var/damage = 1,var/amputation=0)
	if(destspawn) return
	if(override)
		status |= ORGAN_DESTROYED
	if(status & ORGAN_DESTROYED)
		...
		wounds.Cut()
		if (parent && damage && !amputation)
		//if (parent && !amputation)  //MERGE1
			var/datum/wound/W
			if(max_damage < 50)
