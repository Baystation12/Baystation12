...
/obj/structure/kitchenspike/proc/try_spike(var/mob/living/target, var/mob/living/user)
	...
	if(suitable_for_butchery(target))
		...
		if(!do_after_put_in_thing(user, src, target) || occupant || target.stat == CONSCIOUS)
			return
...
/obj/structure/kitchenspike/mob_breakout(mob/living/escapee)
	...
	if(secures_occupant)
		...
		if(!do_after(escapee, DO_AFTER_TIME_SHORT, src, DO_DEFAULT | DO_USER_UNIQUE_ACT | DO_SHOW_PROGRESS))
			return FALSE
...
/obj/structure/kitchenspike/proc/do_butchery_step(var/mob/user, var/next_state, var/butchery_string)
	...
	if(do_after(user, DO_AFTER_TIME_QUICK, src, DO_PUBLIC_UNIQUE) && !QDELETED(last_occupant) && occupant == last_occupant && occupant_state == last_state)
