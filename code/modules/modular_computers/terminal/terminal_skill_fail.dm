...
/datum/terminal_skill_fail/random_ban/execute()
	// julie fix pls ntnet_global.banned_nids |= rand(1,40)
	return ..()
...
/datum/terminal_skill_fail/random_ban/unban/execute()
	// julie fis pls var/id = pick_n_take(ntnet_global.banned_nids)
	//if(id)
	return ..()
...
/datum/terminal_skill_fail/random_ban/purge/execute()
	// julie fix pls ntnet_global.purge_logs()
	return ..()
...
/datum/terminal_skill_fail/random_ban/alarm_reset/execute()
	// julie fix pls ntnet_global.resetIDS()
	return ..()
...
/datum/terminal_skill_fail/random_ban/email_logs/execute()
	// julie fix pls var/datum/computer_file/data/email_account/server = ntnet_global.find_email_by_name(EMAIL_DOCUMENTS)
	for(var/datum/computer_file/data/email_account/email in list()) // julie fix pls ntnet_global.email_accounts
