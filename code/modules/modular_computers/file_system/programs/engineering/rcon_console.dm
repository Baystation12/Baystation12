...
/datum/nano_module/program/rcon/proc/FindDevices()
	known_SMESs = list()
	for(var/obj/machinery/power/smes/buildable/SMES as anything in MACHINES_OF(/obj/machinery/power/smes/buildable))
		if(AreConnectedZLevels(get_host_z(), get_z(SMES)) && SMES.RCon_tag && (SMES.RCon_tag != "NO_TAG") && SMES.RCon)
			known_SMESs.Add(SMES)

	known_breakers = list()
	for(var/obj/machinery/power/breakerbox/breaker as anything in MACHINES_OF(/obj/machinery/power/breakerbox))
