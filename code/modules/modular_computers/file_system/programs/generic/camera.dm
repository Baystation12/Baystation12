...
/datum/nano_module/camera_monitor/proc/set_current(obj/machinery/camera/C)
	...
	current_camera = C
	if(current_camera)
		GLOB.destroyed_event.register(current_camera, src, src::reset_current())
		GLOB.moved_event.register(current_camera, src, src::camera_moved())
...
/datum/nano_module/camera_monitor/proc/reset_current()
	if(current_camera)
		GLOB.destroyed_event.unregister(current_camera, src, src::reset_current())
		GLOB.moved_event.unregister(current_camera, src, src::camera_moved())
		var/mob/living/L = current_camera.loc
		if(istype(L))
