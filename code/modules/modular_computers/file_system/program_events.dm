...
// Called when the network connectivity fails. Computer does necessary checks and only calls this when requires_exonet_feature and similar variables are not met.
/datum/computer_file/program/proc/event_networkfailure(var/background)
	if(!computer)
		return
	computer.kill_program(src, 1)
	if(background)
		computer.visible_error("Network connectivity lost - process [filename].[filetype] (PID [rand(1000,9999)]) terminated.")
	else
		computer.visible_error("FATAL NETWORK ERROR - EXONET connection lost. Please try again later. If problem persists, please contact your system administrator.")
