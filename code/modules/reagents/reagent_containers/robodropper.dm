	...
	afterattack(obj/target, mob/user , flag)
		...
		if(filled)
			...
			if(ismob(target))
				...
				M.attack_log += text("\[[time_stamp()]\] <font color='orange'>Has been squirted ([contained]) with [src.name] by [user.name] ([user.ckey])</font>")
				user.attack_log += text("\[[time_stamp()]\] <font color='red'>Used the [src.name] to squirt [M.name] ([M.key]) with [contained]</font>")
				msg_admin_attack("[user.name] ([user.ckey]) squirted [M.name] ([M.key]) with [src.name], which had [contained] (INTENT: [uppertext(user.a_intent)]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)")
