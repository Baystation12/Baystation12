...
/obj/item/weapon/reagent_containers/pill
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		if(M == user)
			...
			user.attack_log += text("\[[time_stamp()]\] <font colour='red'>Fed [M.name] by [M.name] ([M.ckey]) Reagents: [reagentlist(src)]</font>")
	...
	afterattack(obj/target, mob/user, proximity)
		...
		if(target.is_open_container() != 0 && target.reagents)
			...
			user.attack_log += text("\[[time_stamp()]\] <font colour='red'>Spiked \a [target] with a pill. Reagents: [reagentlist(src)]</font>")
