...
/obj/item/reagent_containers/food/drinks/Initialize() //INF-PORT
	. = ..()
	if(is_open_container())
...
/obj/item/reagent_containers/food/drinks/proc/gulp_whole()
	...
	if(is_open_container())
		if(!reagents || reagents.total_volume == 0)
			...
			playsound(usr, 'proxima/sound/items/drinking.ogg', reagents.total_volume, 1)
			...
			playsound(usr, 'proxima/sound/items/drinking_after.ogg', reagents.total_volume, 1)
