	...
	if(ismob(target))//Blood!
		...
		if(istype(target, /mob/living/carbon))
			...
			if(prob(user.skill_fail_chance(SKILL_MEDICAL, 60, SKILL_BASIC)))
				...
				T.apply_damage(3, DAMAGE_BRUTE, target_zone, damage_flags=DAM_SHARP)
				return
...
/obj/item/reagent_containers/syringe/proc/syringestab(var/mob/living/carbon/target, var/mob/living/carbon/user)
	...
	if(istype(target, /mob/living/carbon/human))
		...
		if (target != user && H.get_blocked_ratio(target_zone, DAMAGE_BRUTE, damage_flags=DAM_SHARP) > 0.1 && prob(50))
		...
		target.apply_damage(3, DAMAGE_BRUTE, target_zone, damage_flags=DAM_SHARP)
