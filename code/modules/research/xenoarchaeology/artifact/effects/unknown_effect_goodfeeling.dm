...
/datum/artifact_effect/goodfeeling/DoEffectTouch(var/mob/user)
	if(user)
		if (istype(user, /mob/living/carbon/human))
			...
			if(prob(50))
				if(prob(75))
					H << "<b><font colour='blue' size='[num2text(rand(1,5))]'><b>[pick(drastic_messages)]</b></font>"
				else
					H << "<font colour='blue'>[pick(messages)]</font>"
		...
		for (var/mob/living/carbon/human/H in range(src.effectrange,holder))
			if(prob(5))
				if(prob(75))
					...
					H << "<font colour='blue' size='[num2text(rand(1,5))]'><b>[pick(drastic_messages)]</b></font>"
