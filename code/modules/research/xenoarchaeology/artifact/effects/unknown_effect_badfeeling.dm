...
/datum/artifact_effect/badfeeling/DoEffectTouch(var/mob/user)
	if(user)
		if (istype(user, /mob/living/carbon/human))
			...
			if(prob(50))
				if(prob(75))
					H << "<b><font colour='red' size='[num2text(rand(1,5))]'><b>[pick(drastic_messages)]</b></font>"
				else
					H << "<font colour='red'>[pick(messages)]</font>"
		...
		for (var/mob/living/carbon/human/H in range(src.effectrange,holder))
			if(prob(5))
				if(prob(75))
					...
					H << "<font colour='red' size='[num2text(rand(1,5))]'><b>[pick(drastic_messages)]</b></font>"
