...
/obj/machinery/computer/rdconsole/New()
	...
	files = new
	if(!id)
		for(var/obj/machinery/r_n_d/server/centcom/S as anything in MACHINES_OF(/obj/machinery/r_n_d/server/centcom))
...
/obj/machinery/computer/rdconsole/OnTopic(user, href_list)
	if(href_list["menu"]) //Switches menu screens. Converts a sent text string into a number. Saves a LOT of code.
		...
		if(!sync)
			...
			spawn(30)
				if(src)
					for(var/obj/machinery/r_n_d/server/S as anything in MACHINES_OF(/obj/machinery/r_n_d/server))
