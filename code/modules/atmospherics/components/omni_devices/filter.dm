...
/obj/machinery/atmospherics/omni/filter
	...
	var/list/filtering = new() //Used to be called filtering. Conflicted with 512.
...
/obj/machinery/atmospherics/omni/filter/error_check()
	if(!input || !output || !filtering)
		return 1
	if(filtering.len < 1) //requires at least 1 filter ~otherwise why are you using a filter?
		return 1
...
/obj/machinery/atmospherics/omni/filter/Process()
	...
	for(var/datum/omni_port/filter_output in filtering)
	...
	if (power_draw >= 0)
		...
		for(var/datum/omni_port/P in filtering)
			if(P.network)
				P.network.update = 1
