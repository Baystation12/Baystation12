...
/datum/category_item/player_setup_item/background/records/content(var/mob/user)
	. = list()
	. += "<br/><b>Records</b>:<br/>"
	if(user.client.is_banned(BAN_RECORDS))
...
/datum/category_item/player_setup_item/background/records/OnTopic(var/href,var/list/href_list, var/mob/user)
	if (href_list["set_public_record"])
		...
		if (!isnull(new_public) && !user.client.is_banned(BAN_RECORDS) && CanUseTopic(user))
		...
		if(!isnull(new_medical) && !user.client.is_banned(BAN_RECORDS) && CanUseTopic(user))
		...
		if(!isnull(new_general) && !user.client.is_banned(BAN_RECORDS) && CanUseTopic(user))
		...
		if(!isnull(sec_medical) && !user.client.is_banned(BAN_RECORDS) && CanUseTopic(user))
