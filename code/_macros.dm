...
#define show_browser(target, content, title)  to_target(target, browse(isfile(content) ? content : findtext_char(content, "<html><head>") ? replacetext_char(content, "<html><head>", "<html><head><meta charset='utf-8'>") : findtext_char(content, "<head>") ? replacetext_char(content, "<head>", "<head><meta charset='utf-8'>") : "<head><meta charset='utf-8'>[content]", title))	// Proxima
