...
/datum/controller/air_system/proc/Tick()
	...
	if(tiles_to_update.len)
		...
		if(updated != updating.len)
			...
			to_chat(world, "<span class='danger'>[tick_progress]</span>")
			. = 0
		#endif
...
/datum/controller/air_system/proc/mark_edge_active(connection_edge/E)
	...
	if(istype(E, /connection_edge/zone/))
		...
		to_chat(world, "ZASDBG: Active edge! Areas: [get_area(pick(ZE.A.contents))] / [get_area(pick(ZE.B.contents))]")
	else
		to_chat(world, "ZASDBG: Active edge! Area: [get_area(pick(E.A.contents))]")
	#endif
