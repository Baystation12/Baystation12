...
/datum/unit_test/cryopod_comp_check/start_test()
	...
	for(var/obj/machinery/cryopod/C as anything in MACHINES_OF(/obj/machinery/cryopod))
		if(!C.control_computer)
			log_bad("[get_area(C)] lacks a cryopod control computer while holding a cryopod.")
			pass = FALSE

	for(var/obj/machinery/computer/cryopod/C as anything in MACHINES_OF(/obj/machinery/computer/cryopod))
...
/datum/unit_test/shutoff_valves_shall_connect_to_two_different_pipe_networks/start_test()
	var/failures = 0
	for(var/obj/machinery/atmospherics/valve/shutoff/SV as anything in MACHINES_OF(/obj/machinery/atmospherics/valve/shutoff))
		SV.close()
	for(var/obj/machinery/atmospherics/valve/shutoff/SV as anything in MACHINES_OF(/obj/machinery/atmospherics/valve/shutoff))
...
/datum/unit_test/station_power_terminals_shall_be_wired/start_test()
	var/failures = 0
	for(var/obj/machinery/power/terminal/term as anything in MACHINES_OF(/obj/machinery/power/terminal))
