...
/atom/movable/screen/plane_master/rendering_plate/game_world/Initialize(mapload)
	. = ..()
	//If you want effects that affect entire game world, add them here
	filters += filter(type = "displace", render_source = GRAVITY_PULSE_RENDER_TARGET, size = 10)
...
/atom/movable/screen/plane_master/proc/relay_render_to_plane(mob/mymob, relay_plane)
	...
	relay.layer = (plane + abs(LOWEST_PLANE))*0.5 //layer must be positive but can be a decimal
