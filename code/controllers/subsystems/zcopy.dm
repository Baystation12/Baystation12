	...
	for (var/z in 1 to world.maxz)
		if (!HasAbove(z))
			...
			if (z - start_zlev > OPENTURF_MAX_DEPTH)
				LOG_WARNING("Zcopy levels [start_zlev] through [z] exceed maximum depth of [OPENTURF_MAX_DEPTH]; layering may behave strangely in this Z-stack.")
			else if (z - start_zlev > 1)
				LOG_DEBUG("Zcopy Found stack [start_zlev] -> [z] = [z - start_zlev + 1] zl")
			start_zlev = z + 1
	LOG_DEBUG("Zcopy level maximums: [json_encode(zlev_maximums)]")
