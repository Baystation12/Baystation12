	...
	if (istype(M, /mob/living/carbon/human))
		...
		if(affecting.is_bandaged())
			...
			for (var/datum/wound/W in affecting.wounds)
				...
				if(used == amount)
					break
				if(!do_after(user, W.damage / 5, M, DO_DEFAULT & ~DO_BOTH_UNIQUE_ACT))
					break
...
/obj/item/stack/medical/ointment/attack(var/mob/living/carbon/M, var/mob/user)
	...
	if (istype(M, /mob/living/carbon/human))
		...
		if(affecting.is_salved())
			...
			playsound(src, pick(apply_sounds), 25)
			if(!do_after(user, 1 SECOND, M, DO_DEFAULT & ~DO_BOTH_UNIQUE_ACT))
				return 1
...
/obj/item/stack/medical/splint/attack(var/mob/living/carbon/M, var/mob/user)
	...
	if (istype(M, /mob/living/carbon/human))
		...
		if(user.do_skilled(5 SECONDS, SKILL_MEDICAL, M, do_flags = DO_DEFAULT & ~DO_BOTH_UNIQUE_ACT))
