...
/obj/item/weapon/spirit_board/examine(var/user)
	..()
	to_chat("[initial(desc)] The planchette is sitting at \"[planchette]\".")
...
/obj/item/weapon/spirit_board/proc/spirit_board_checks(mob/M)
	...
	if(light_amount > 0.2)
		to_chat(M, "<span class='warning'>It's too bright here to use \the [src.name]!</span>")
		return 0
	...
	for(var/mob/living/L in orange(1,src))
		if(L.ckey && L.client)
			if(L.client.is_afk(300) || L.incapaciated())//no playing with braindeads or corpses or handcuffed dudes.
				to_chat(M, "<span class='warning'>[L] doesn't seem to be paying attention...</span>")
			else
				users_in_range++

	if(users_in_range < 2)
		to_chat(M, "<span class='warning'>There aren't enough people to use \the [src.name]!</span>")
		return 0
