		...
		if(istype(H))
			...
			if(H != user)	//can't look into your own eyes buster
				...
				if(vision.is_bruised())
					...
					user << "<span class='notice'>[M]'s pupils react slower than normally.</span>"
				...
				if(M.reagents.has_any_reagent(pinpoint) || H.ingested.has_any_reagent(pinpoint))
					user << "<span class='notice'>[M]'s pupils are already pinpoint and cannot narrow any more.</span>"
				else if(M.reagents.has_any_reagent(dilating) || H.ingested.has_any_reagent(dilating))
					user << "<span class='notice'>[M]'s pupils narrow slightly, but are still very dilated.</span>"
				else
