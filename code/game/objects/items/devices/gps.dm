...
/obj/item/device/gps/Initialize()
	...
	GLOB.moved_event.register(holder, src, src::update_holder())
	compass = new(src)
	update_holder()
	update_icon()
...
/obj/item/device/gps/proc/update_holder(force_clear = FALSE)
	...
	if(!force_clear && istype(loc, /mob))
		holder = loc
		GLOB.moved_event.register(holder, src, src::update_compass())
		GLOB.dir_set_event.register(holder, src, src::update_compass())
...
/obj/item/device/gps/Destroy()
	...
	is_in_processing_list = FALSE
	global.all_gps_units -= src
	GLOB.moved_event.unregister(holder, src, src::update_holder())
...
/obj/item/device/gps/emp_act(severity)
	...
	emped = TRUE
	update_icon()
	addtimer(new Callback(src, src::reset_emp()), duration)
