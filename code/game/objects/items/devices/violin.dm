...
/obj/item/device/violin/proc/playnote(var/note as text)
	...
	// hearers(15, get_turf(src)) << sound(soundfile,0,null,null,67)  // Removed this code in favor of playsound_local

	var/turf/source = get_turf(src)
	for(var/mob/M in hearers(15, source))
		M.playsound_local(source, file(soundfile), 67, falloff = 2)
