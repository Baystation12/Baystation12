/obj/item/weapon/rcdadmin
	...
	throw_range = 5
	w_class = 3.0
	matter = list("metal" = 999999)
	var/datum/effect/effect/system/spark_spread/spark_system
	var/stored_matter = 9999999
	var/working = 0
	var/mode = 1
	var/canRwall = 0
	var/disabled = 0
			...
			if(1)
				...
				if(istype(A, /turf/simulated/floor))
					if(checkResource(1, user))
						...
						playsound(src.loc, 'sound/machines/click.ogg', 50, 1)
						if(do_after(user, 1))
							if(!useResource(1, user)) return 0
							activate()
...
/obj/item/weapon/rcdadmin/proc/useResource(var/amount, var/mob/user)
...
/obj/item/weapon/rcdadmin/proc/checkResource(var/amount, var/mob/user)
