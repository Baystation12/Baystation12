	...
	if(user.zone_sel.selecting == "head" || istype(M, /mob/living/carbon/slime))
		...
		switch(M:brain_op_stage)
			...
			if(3.0)
				...
				user.attack_log += "\[[time_stamp()]\]<font colour='red'> Debrained [M.name] ([M.ckey]) with [src.name] (INTENT: [uppertext(user.a_intent)])</font>"
				M.attack_log += "\[[time_stamp()]\]<font colour='orange'> Debrained by [user.name] ([user.ckey]) with [src.name] (INTENT: [uppertext(user.a_intent)])</font>"

				log_attack("<font colour='red'>[user.name] ([user.ckey]) debrained [M.name] ([M.ckey]) with [src.name] (INTENT: [uppertext(user.a_intent)])</font>")
