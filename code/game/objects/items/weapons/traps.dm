...
/obj/item/beartrap/user_unbuckle_mob(mob/user as mob)
	if(buckled_mob && can_use(user))
		...
		if(do_after(user, DO_AFTER_TIME_SHORT, src, DO_PUBLIC_UNIQUE, do_skill = SKILL_HAULING, delay_flags = DO_AFTER_TIME_FLAG_USER_SKILL))
...
/obj/item/beartrap/attack_self(mob/user as mob)
	...
	if(!deployed && can_use(user))
		...
		if (do_after(user, DO_AFTER_TIME_SHORT, src, do_skill = SKILL_HAULING, delay_flags = DO_AFTER_TIME_FLAG_USER_SKILL) && user.unEquip(src))
			user.visible_message(
...
/obj/item/beartrap/attack_hand(mob/user as mob)
	if(buckled_mob)
		...
		if(do_after(user, DO_AFTER_TIME_SHORT, src, do_skill = SKILL_HAULING, delay_flags = DO_AFTER_TIME_FLAG_USER_SKILL))
			user.visible_message(
