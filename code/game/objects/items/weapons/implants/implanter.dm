	...
	if (user && src.imp)
		...
		if (T1 && ((M == user) || do_after(user, 50)))
			if(user && M && (get_turf(M) == T1) && src && src.imp)
				...
				M.attack_log += text("\[[time_stamp()]\] <font colour='orange'> Implanted with [src.name] ([src.imp.name])  by [user.name] ([user.ckey])</font>")
				user.attack_log += text("\[[time_stamp()]\] <font colour='red'>Used the [src.name] ([src.imp.name]) to implant [M.name] ([M.ckey])</font>")
