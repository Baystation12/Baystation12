...
/obj/item/implant/explosive/activate()
	...
	if(ismob(imp_in))
		...
		log_and_message_admins("Explosive implant triggered in [imp_in] ([imp_in.key]). (<A HREF='?_src_=holder;admin_goto=1;X=[imp_in.x];Y=[imp_in.y];Z=[imp_in.z]'>JMP</a>) ")
	else
		audible_message("<span class='warning'>[src] beeps omniously!</span>")
		log_and_message_admins("Explosive implant triggered in [T.loc]. (<A HREF='?_src_=holder;admin_goto=1;X=[T.x];Y=[T.y];Z=[T.z]'>JMP</a>) ")
