...
/obj/item/storage/mirror/proc/on_flick() //Have to hide the effect
	if(istype(reflection))
		reflection.alpha = 0
		addtimer(new Callback(reflection, TYPE_PROC_REF(/obj/effect/reflection, reset_alpha)), 15, TIMER_UNIQUE | TIMER_OVERRIDE)
...
/obj/effect/reflection
	...
	var/alpha_icon = 'icons/obj/structures/mirror.dmi'
