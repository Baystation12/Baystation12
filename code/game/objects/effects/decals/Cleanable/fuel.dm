/obj/decal/cleanable/liquid_fuel
...
/obj/decal/cleanable/liquid_fuel/proc/Spread(exclude=list())
	...
	for(var/d in GLOB.cardinal)
		...
		if(origin.CanPass(null, target, 0, 0) && target.CanPass(null, origin, 0, 0))
			var/obj/decal/cleanable/liquid_fuel/other_fuel = locate() in target
			if(other_fuel)
				...
				new/obj/decal/cleanable/liquid_fuel(target, amount*0.25,1)
			amount *= 0.75
...
/obj/decal/cleanable/liquid_fuel/Initialize(mapload, amt=1, nologs=FALSE)
	...
	for(var/obj/decal/cleanable/liquid_fuel/other in loc)
		if(other != src)
