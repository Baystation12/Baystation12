...
/obj/structure/railing/grab_attack(obj/item/grab/G)
	. = TRUE
...
/obj/structure/railing/use_tool(obj/item/W, mob/user)
	// Dismantle
	if(isWrench(W))
		if(!anchored)
			playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
			if(do_after(user, 20, src) && !anchored)
	...
	if(isWelder(W))
		...
		if(F.isOn())
			...
			if(do_after(user, 20, src) && health_damaged())
