...
/obj/structure/iv_drip/proc/rip_out()
	...
	attached.apply_damage(1, DAMAGE_BRUTE, pick(BP_R_ARM, BP_L_ARM), damage_flags=DAM_SHARP)
	attached = null
...
/proc/do_IV_hookup(mob/living/carbon/human/target, mob/user, obj/IV)
	...
	if(prob(user.skill_fail_chance(SKILL_MEDICAL, 80, SKILL_BASIC)))
		...
		target.apply_damage(2, DAMAGE_BRUTE, pick(BP_R_ARM, BP_L_ARM), damage_flags=DAM_SHARP)
		return FALSE
