...
/obj/structure/fence/attack_slime(mob/user as mob)
...
/obj/structure/fence/attack_animal(var/mob/living/simple_animal/M as mob)
...
/obj/structure/fence/bullet_act(var/obj/item/projectile/Proj)
	...
	//Tasers and the like should not damage fences.
...
/obj/structure/fence/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(iswirecutter(W))
		...
		if(!shock(user, 90))
			...
			anchored = !anchored
			user.visible_message("<span class='notice'>[user] [anchored ? "fastens" : "unfastens"] the fence.</span>", \
								 "<span class='notice'>You have [anchored ? "fastened the fence to" : "unfastened the grill from"] the floor.</span>")
			return
...
/obj/structure/fence/proc/shock(mob/user as mob, prb)

	if(!anchored || destroyed)		// anchored/destroyed fences are never connected
		return 0
	if(!prob(prb))
		return 0
