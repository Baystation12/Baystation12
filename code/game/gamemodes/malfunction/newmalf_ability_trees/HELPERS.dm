...
/proc/announce_hack_failure(var/mob/living/silicon/ai/user = null, var/text)
	...
	switch(user.hack_fails)
		...
		if(2)
			fulltext = "Мы обнаружили еще одну попытку взлома. Он был нацелен на [text]. Злоумышленник почти получил контроль над системой, поэтому нам пришлось отключить их. Мы частично завершили нашу трассировку, и, похоже, она исходит либо из [GLOB.using_map.station_name], либо из объекта в непосредственной близости от него."
		if(3)
			fulltext = "Была обнаружена еще одна попытка взлома, на этот раз нацеленная на [text]. Мы уверены, что злоумышленник проник в сеть через терминал, расположенный где-то на [GLOB.using_map.station_name]."
		if(4)
