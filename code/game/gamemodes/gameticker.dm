	...
	do
		if(!gamemode_voted)
			...
			if(!isnull(secondary_mode))
				...
				secondary_mode = null
				to_chat(world, "Trying to start the second top game mode...")
				if(!hide_mode)
					to_chat(world, "<b>The game mode is now: [master_mode]</b>")
			else if(!isnull(tertiary_mode))
				master_mode = tertiary_mode
				tertiary_mode = null
				to_chat(world, "Trying to start the third top game mode...")
				if(!hide_mode)
					to_chat(world, "<b>The game mode is now: [master_mode]</b>")
			else
				master_mode = "extended"
				to_chat(world, "<b>Forcing the game mode to extended...</b>")
...
/datum/controller/gameticker/proc/setup()
	...
	spawn(0)//Forking here so we dont have to wait for this to finish
		...
		playsound(world, sound('sound/AI/welcome.ogg'))// Skie
			...
			if(1)	//nuke was nearby but (mostly) missed
				...
				switch( override )
					if("mercenary") //Nuke wasn't on station when it blew up
						...
						sleep(35)
						playsound(world, sound('sound/effects/explosionfar.ogg'))
