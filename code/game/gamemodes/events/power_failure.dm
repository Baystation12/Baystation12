...
/proc/power_failure(announce = 1, severity = 2, list/affected_z_levels)
	...
	for(var/obj/machinery/power/apc/C as anything in MACHINES_OF(/obj/machinery/power/apc))
...
/proc/power_restore(announce = 1)
	...
	for(var/obj/machinery/power/smes/S as anything in MACHINES_OF(/obj/machinery/power/smes))
		S.failure_timer = 0
		S.charge = S.capacity
		S.update_icon()
		S.power_change()
