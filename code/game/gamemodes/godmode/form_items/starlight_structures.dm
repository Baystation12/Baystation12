...
/obj/structure/deity/gateway/attack_deity(mob/living/deity/deity)
	var/list/html = list()
...
/obj/structure/deity/gateway/CanUseTopic(mob/user)
	if(linked_god && (user == linked_god || user.loc == linked_god.loc))
		return STATUS_INTERACTIVE
	return STATUS_CLOSE

/obj/structure/deity/gateway/proc/stop_looking_for(successful)
	if(looking_for)
		if(!successful)
			to_chat(linked_god, SPAN_WARNING("\The [src] did not find any [looking_for]. You may try again if you wish."))
		looking_for = null

/obj/structure/deity/gateway/OnTopic(mob/user, list/href_list)
		...
		if(looking_for)
			...
			for(var/l in get_turf(linked_god))
				if(istype(l, /mob/living/starlight_soul))
					to_chat(l, SPAN_NOTICE("\The [src] is looking for a soul to become a [looking_for]. Accept? (<a href='?\ref[src];accept=[looking_for]'>Yes</a>)"))
			addtimer(new Callback(src, .proc/stop_looking_for, FALSE), 30 SECONDS)
