...
/obj/structure/deity/altar/Process()
	...
	if(!cycles_before_converted)
		visible_message("For one thundering moment, \the [target] cries out in pain before going limp and broken.")
...
/obj/structure/deity/altar/OnTopic(user, list/href_list)
	if(href_list["resist"])
		...
		M.visible_message(
			SPAN_WARNING("\The [M] writhes on top of \the [src]!"),
			SPAN_NOTICE("You struggle against the intruding thoughts, keeping them at bay!")
		)
		to_chat(linked_god, SPAN_WARNING("\The [M] slows its conversion through willpower!"))
		cycles_before_converted++
		if(prob(50))
			to_chat(M, SPAN_WARNING("The mental strain is too much for you! You feel your body weakening!"))
			M.adjustToxLoss(15)
			M.adjustHalLoss(30)
