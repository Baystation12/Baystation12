...
/mob/proc/make_vampire()
	...
	if (!mind.vampire)
		...
		if(ishuman(src))
			...
			H.current_grab_type = /obj/item/grab/normal/vamp //Incompatible with Nabbers. Sorry.
...
/mob/proc/vampire_start_frenzy(var/force_frenzy = 0)
	...
	if (prob(probablity))
		...
		add_spell(/spell/targeted/vampire/grapple)
...
/mob/proc/vampire_stop_frenzy(var/force_stop = 0)
	...
	if (prob(force_stop ? 100 : vampire.blood_usable))
		...
		remove_spell(/spell/targeted/vampire/grapple)
...
//Handle blood calculations. Positive values add blood, negatives do the reverse.

/mob/proc/draw_usable_blood(var/adjustment_val, var/mob/user) //Accepts negative values. No support for adding usable blood (Values that don't increase your bloodcap)

	if(adjustment_val > 0)//Gaining fresh blood.
		user.mind.vampire.blood_total += adjustment_val
		user.mind.vampire.blood_usable += adjustment_val
		to_chat(user, "<span class='notice'>You have accumulated [adjustment_val] [adjustment_val > 1 ? "units" : "unit"] of blood.</span>")

	to_chat(user, "<span class='notice'> And have [user.mind.blood_usable] left to use.</span>")
