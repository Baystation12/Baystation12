...
/obj/item/mecha_parts/mecha_equipment/tool/sleeper/Topic(href,href_list)
	..()
	var/datum/topic_input/list_filter_list = new /datum/topic_input(href,href_list)
	if(list_filter_list.get("eject"))
		go_out()
	if(list_filter_list.get("view_stats"))
		chassis.occupant << browse(get_occupant_stats(),"window=msleeper")
		onclose(chassis.occupant, "msleeper")
		return
	if(list_filter_list.get("inject"))
		inject_reagent(list_filter_list.getType("inject",/datum/reagent),list_filter_list.getObj("source"))
	return
...
/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun/Topic(href,href_list)
	..()
	var/datum/topic_input/filter_list = new (href,href_list)
	if(filter_list.get("toggle_mode"))
		mode = !mode
		update_equip_info()
		return
	if(filter_list.get("select_reagents"))
		...
		for(var/i=1 to known_reagents.len)
			if(m>=synth_speed)
				break
			var/reagent = filter_list.get("reagent_[i]")
		...
		return
	if(filter_list.get("show_reagents"))
		chassis.occupant << browse(get_reagents_page(),"window=msyringegun")
	if(filter_list.get("purge_reagent"))
		var/datum/reagent/R = locate(filter_list.get("purge_reagent")) in reagents.reagent_list
		if(R)
			reagents.del_reagent(R.type)
		return
	if(filter_list.get("purge_all"))
