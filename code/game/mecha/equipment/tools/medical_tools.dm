...
/obj/item/mecha_parts/mecha_equipment/tool/sleeper/Topic(href,href_list)
	...
	if(filter2.get("eject"))
		go_out()
	if(filter2.get("view_stats"))
		chassis.occupant << browse(get_occupant_stats(),"window=msleeper")
		onclose(chassis.occupant, "msleeper")
		return
	if(filter2.get("inject"))
		inject_reagent(filter2.getType("inject",/datum/reagent),filter2.getObj("source"))
	return
...
/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun/Topic(href,href_list)
	..()
	var/datum/topic_input/filter2 = new (href,href_list)
	if(filter2.get("toggle_mode"))
		mode = !mode
		update_equip_info()
		return
	if(filter2.get("select_reagents"))
		...
		for(var/i=1 to known_reagents.len)
			if(m>=synth_speed)
				break
			var/reagent = filter2.get("reagent_[i]")
		...
		return
	if(filter2.get("show_reagents"))
		chassis.occupant << browse(get_reagents_page(),"window=msyringegun")
	if(filter2.get("purge_reagent"))
		var/datum/reagent/R = locate(filter2.get("purge_reagent")) in reagents.reagent_list
		if(R)
			reagents.del_reagent(R.type)
		return
	if(filter2.get("purge_all"))
