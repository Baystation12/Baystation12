...
/obj/mecha/Topic(href, href_list)
	...
	if(usr.stat > 0)
		return
	var/datum/topic_input/filter2 = new /datum/topic_input(href,href_list)
	if(href_list["select_equip"])
		if(usr != src.occupant)	return
		var/obj/item/mecha_parts/mecha_equipment/equip = filter2.getObj("select_equip")
		if(equip)
			src.selected = equip
	...
	if(href_list["rfreq"])
		if(usr != src.occupant)	return
		var/new_frequency = (radio.frequency + filter2.getNum("rfreq"))
	...
	if(href_list["req_access"] && add_req_access)
		...
		output_access_dialog(filter2.getObj("id_card"),filter2.getMob("user"))
		return
	if(href_list["maint_access"] && maint_access)
		if(!in_range(src, usr))	return
		var/mob/user = filter2.getMob("user")
		if(user)
			if(state==0)
				state = 1
			...
			output_maintenance_dialog(filter2.getObj("id_card"),user)
		return
	...
	if(href_list["remove_passenger"] && state >= 1)
		...
		return
	if(href_list["add_req_access"] && add_req_access && filter2.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access += filter2.getNum("add_req_access")
		output_access_dialog(filter2.getObj("id_card"),filter2.getMob("user"))
		return
	if(href_list["del_req_access"] && add_req_access && filter2.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access -= filter2.getNum("del_req_access")
		output_access_dialog(filter2.getObj("id_card"),filter2.getMob("user"))
		return
