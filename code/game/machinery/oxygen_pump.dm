...
/obj/machinery/oxygen_pump/attackby(obj/item/W as obj, mob/user as mob)
	if(isScrewdriver(W))
		...
		user.visible_message("<span class='notice'>\The [user] [GET_FLAGS(stat, MACHINE_STAT_MAINT) ? "opens" : "closes"] \the [src].</span>", "<span class='notice'>You [GET_FLAGS(stat, MACHINE_STAT_MAINT) ? "open" : "close"] \the [src].</span>")
		if(GET_FLAGS(stat, MACHINE_STAT_MAINT))
