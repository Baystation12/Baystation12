...
/obj/machinery/oxygen_pump/New()
	...
	GLOB.destroyed_event.register(tank, src, src::fix_deleted_tank())
	GLOB.destroyed_event.register(contained, src, src::fix_deleted_mask())

/obj/machinery/oxygen_pump/Destroy()
	if(breather)
		detach_mask(breather)
	GLOB.destroyed_event.unregister(tank, src, src::fix_deleted_tank())
	GLOB.destroyed_event.unregister(contained, src, src::fix_deleted_mask())
	QDEL_NULL(tank)
	QDEL_NULL(contained)
	return ..()
...
/obj/machinery/oxygen_pump/proc/fix_deleted_tank(obj/item/tank/_tank)
	GLOB.destroyed_event.unregister(_tank, src, src::fix_deleted_tank())
	tank = new spawn_type(src)
	GLOB.destroyed_event.register(tank, src, src::fix_deleted_tank())
...
/obj/machinery/oxygen_pump/proc/fix_deleted_mask(obj/item/clothing/mask/breath/_mask)
	GLOB.destroyed_event.unregister(_mask, src, src::fix_deleted_mask())
...
/obj/machinery/oxygen_pump/proc/attach_mask(mob/living/carbon/C)
	if(C && istype(C))
		...
		if(tank)
			tank.forceMove(C)
		breather = C
		GLOB.destroyed_event.register(breather, src, src::detach_mask())
...
/obj/machinery/oxygen_pump/proc/detach_mask(mob/user)
	...
	GLOB.destroyed_event.unregister(breather, src, src::detach_mask())
	breather = null
