...
/obj/machinery/porta_turret/post_health_change(health_mod, prior_health, damage_type)
	...
	if (health_mod < 0)
		if (!emagged && enabled)
			attacked = TRUE
			addtimer(new Callback(src, src::timer_attacked()), 6 SECONDS, TIMER_UNIQUE | TIMER_OVERRIDE)
		if (prob(45))
			spark_system.start()
...
/obj/machinery/porta_turret/emp_act(severity)
	...
	addtimer(new Callback(src, /obj/machinery/porta_turret::enable()), rand(60*power,600*power))
