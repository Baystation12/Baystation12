...
/obj/machinery/tele_beacon/emp_act(severity)
	...
	if (use_power && !stat)
		...
		addtimer(new Callback(src, src::emp_act_end()), emp_time, TIMER_UNIQUE | TIMER_OVERRIDE)
...
/datum/wires/tele_beacon/UpdatePulsed(index)
	...
	switch (index)
		if (TELEBEACON_WIRE_POWER)
			...
			addtimer(new Callback(src, src::ResetPulsed()), rand(15 SECONDS, 45 SECONDS), TELEBEACON_WIRE_POWER)
