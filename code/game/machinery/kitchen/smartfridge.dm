...
/obj/machinery/smartfridge
	...
	var/scan_id = 1
	var/is_secure = 0
	/// `type path = ammount`. Defaults to 1 if no ammount equals `null`
	var/startswith = list()
...
/obj/machinery/smartfridge/proc/accept_check(item)
	if(isobj(item))
		if(is_type_in_list(item, accepted_types))
			return TRUE
	else if(ispath(item))
		if(is_path_in_list(item, accepted_types))
			return TRUE
	return FALSE
