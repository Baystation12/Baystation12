...
/obj/machinery/gibber/proc/move_into_gibber(mob/user,mob/living/victim)
	...
	if(do_after(user, 3 SECONDS, src, DO_PUBLIC_UNIQUE) && victim.Adjacent(src) && user.Adjacent(src) && victim.Adjacent(user) && !occupant)
		...
		src.occupant = victim
		GLOB.destroyed_event.register(occupant, src, src::occupant_destroyed())
		update_icon()
...
/obj/machinery/gibber/proc/occupant_destroyed(mob/_occupant)
	if (occupant == _occupant)
		occupant = null
		update_icon()
	GLOB.destroyed_event.unregister(_occupant, src, src::occupant_destroyed())
...
/obj/machinery/gibber/proc/go_out()
	...
	GLOB.destroyed_event.unregister(occupant, src, src::occupant_destroyed())
...
/obj/machinery/gibber/proc/startgibbing(mob/user as mob)
	...
	addtimer(new Callback(src, src::finish_gibbing()), gib_time)
