...
/obj/machinery/door/airlock
	...
	var/open_failure_access_denied = 'sound/machines/deniedbeep.ogg'
	var/open_failure_access_denied_high_sec = 'sound/machines/buzz-two.ogg'
		...
		if("deny")
			...
			if(density && arePowerSystemsOn())
				...
				if(secured_wires && world.time > next_clicksound)
					...
					playsound(loc, open_failure_access_denied_high_sec, 50, 0)
			update_icon(AIRLOCK_CLOSED)
		if("emag")
