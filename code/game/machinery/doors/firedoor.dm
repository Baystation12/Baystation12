...
/obj/machinery/door/firedoor/attack_hand(mob/user)
	...
	if(blocked)
		...
		return
	if(density && (stat & (MACHINE_STAT_BROKEN|MACHINE_STAT_NOPOWER))) //can still close without power
...
/obj/machinery/door/firedoor/attackby(obj/item/C, mob/user)
	...
	if(isCrowbar(C) || istype(C,/obj/item/material/twohanded/fireaxe))
		...
		if(do_after(user, 3 SECONDS, src, DO_REPAIR_CONSTRUCT))
			if(isCrowbar(C))
				if(stat & (MACHINE_STAT_BROKEN|MACHINE_STAT_NOPOWER) || !density)
					user.visible_message(
...
/obj/machinery/door/firedoor/proc/can_safely_open()
	...
	for(var/dir in GLOB.cardinal)
		...
		if(!alarm || (alarm.stat & (MACHINE_STAT_NOPOWER|MACHINE_STAT_BROKEN)))
			return
	return TRUE
