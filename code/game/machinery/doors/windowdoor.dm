...
/obj/machinery/door/window/Bumped(atom/movable/AM as mob|obj)
	if (!( ismob(AM) ))
		...
		if(istype(bot))
			if(density && src.check_access(bot.botcard))
				open()
				addtimer(new Callback(src, src::close()), 5 SECONDS, TIMER_UNIQUE | TIMER_OVERRIDE)
		return
	...
	if (src.density && (!issmall(M) || ishuman(M) || issilicon(M)) && src.allowed(AM))
		...
		addtimer(new Callback(src, src::close()), open_timer, TIMER_UNIQUE | TIMER_OVERRIDE)
	return
...
/obj/machinery/door/window/open()
	...
	addtimer(new Callback(src, src::open_final()), 1 SECOND, TIMER_UNIQUE | TIMER_OVERRIDE)
...
/obj/machinery/door/window/close()
	...
	addtimer(new Callback(src, src::close_final()), 1 SECOND, TIMER_UNIQUE | TIMER_OVERRIDE)
	return TRUE
...
/obj/machinery/door/window/emag_act(remaining_charges, mob/user)
	...
	if (density)
		...
		addtimer(new Callback(src, src::open()), 6, TIMER_UNIQUE | TIMER_OVERRIDE)
	return TRUE
