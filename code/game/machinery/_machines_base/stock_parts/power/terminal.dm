...
/obj/item/stock_parts/power/terminal/proc/set_terminal(obj/machinery/machine, obj/machinery/power/new_terminal)
	...
	terminal.master = src
	GLOB.destroyed_event.register(terminal, src, src::unset_terminal())

	set_extension(src, /datum/extension/event_registration/shuttle_stationary, GLOB.moved_event, machine, src::machine_moved(), get_area(src))
...
/obj/item/stock_parts/power/terminal/proc/machine_moved(obj/machinery/machine, turf/old_loc, turf/new_loc)
	if(!terminal)
		GLOB.moved_event.unregister(machine, src, src::machine_moved())
		return
