			...
			for(var/mob/living/silicon/robot/R in mob_list)
				...
				if (istype(user, /mob/living/silicon))
					if((user.mind.special_role && user.mind.original == user) && !R.emagged)
						dat += "<A href='?src=\ref[src];magbot=\ref[R]'>(<font colour=blue><i>Hack</i></font>)</A> "
				dat += "<A href='?src=\ref[src];stopbot=\ref[R]'>(<font colour=green><i>[R.canmove ? "Lockdown" : "Release"]</i></font>)</A> "
				dat += "<A href='?src=\ref[src];killbot=\ref[R]'>(<font colour=red><i>Destroy</i></font>)</A>"
				dat += "<BR>"
