...
/obj/structure/AIcore/use_tool(obj/item/P, mob/user)
	if(!authorized)
	...
	switch(state)
		...
		if(2)
			...
			if(isCoil(P))
				...
				playsound(loc, 'sound/items/Deconstruct.ogg', 50, 1)
				if (do_after(user, 20, src) && state == 2 && C.use(5))
					state = 3
					icon_state = "3"
		...
		if(3)
			...
			if(istype(P, /obj/item/stack/material))
				...
				if(RG.material.name == MATERIAL_GLASS && RG.reinf_material)
					...
					playsound(loc, 'sound/items/Deconstruct.ogg', 50, 1)
					if (do_after(user, 20,src) && state == 3 && RG.use(2))
...
/obj/structure/AIcore/deactivated/use_tool(obj/item/W, mob/user)
