...
/obj/machinery/alarm/initialize()
	...
	TLV["chlorine"] =		list(-1.0, -1.0, 0.2, 0.5) // Partial pressure, kpa
...
/obj/machinery/alarm/proc/overall_danger_level(var/datum/gas_mixture/environment)
	...
	chlorine_dangerlevel = get_danger_level(environment.gas["chlorine"]*partial_pressure, TLV["chlorine"])
...
/obj/machinery/alarm/proc/populate_status(var/data)
	...
	if(total)
		...
		environment_data[++environment_data.len] = list("name" = "Chlorine", "value" = environment.gas["chlorine"] / total * 100, "unit" = "%", "danger_level" = chlorine_dangerlevel)
	...
	switch(screen)
		...
		if(AALARM_SCREEN_SCRUB)
			...
			for(var/id_tag in alarm_area.air_scrub_names)
				...
				scrubbers[scrubbers.len]["filters"] += list(list("name" = "Chlorine"	, 	"command" = "cl_scrub","val" = info["filter_chlorine"]))
