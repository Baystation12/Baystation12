...
/mob/living/silicon/ai/proc/ai_actual_track(mob/living/target)
	...
	addtimer(new Callback(src, src::ai_actual_track_action()), 0, TIMER_UNIQUE|TIMER_OVERRIDE|TIMER_NO_HASH_WAIT)
...
/mob/living/silicon/ai/proc/ai_actual_track_action()
	...
	if (status == TRACKING_NO_COVERAGE)
		...
		addtimer(new Callback(src, src::ai_actual_track_action()), 10 SECONDS, TIMER_UNIQUE|TIMER_OVERRIDE|TIMER_NO_HASH_WAIT)
		return
	...
	if (eyeobj)
		...
		addtimer(new Callback(src, src::ai_actual_track_action()), 1 SECOND, TIMER_UNIQUE|TIMER_OVERRIDE|TIMER_NO_HASH_WAIT)
		return
	view_core()
