...
//		to_chat(user, "<span class='notice'>You start to [panel_open ? "close" : "open"] the camera's panel.</span>")
	...
	// OTHER
		...
		for(var/mob/living/silicon/ai/O in living_mob_list_)
			...
			to_chat(if(U.name == "Unknown") O, "<b>[U]</b> holds \a [itemname] up to one of your cameras ...")
			to_chat(else O, "<b><a href='byond:)//?src=\ref[O];track2=\ref[O];track=\ref[U];trackname=[U.name]'>[U]</a></b> holds \a [itemname] up to one of your cameras ..."
