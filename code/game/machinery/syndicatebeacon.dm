...
/obj/machinery/syndicate_beacon
	...
	attack_hand(var/mob/user as mob)
		usr.set_machine(src)
		var/dat = "<font colour=#005500><i>Scanning [pick("retina pattern", "voice print", "fingerprints", "dna sequence")]...<br>Identity confirmed,<br></i></font>"
		if(istype(user, /mob/living/carbon/human) || istype(user, /mob/living/silicon/ai))
			if(is_special_character(user))
				dat += "<font colour=#07700><i>Operative record found. Greetings, Agent [user.name].</i></font><br>"
			else if(charges < 1)
				...
				if(user.gender == FEMALE)
					honorific = "Ms."
				dat += "<font colour=red><i>Identity not found in operative database. What can the Syndicate do for you today, [honorific] [user.name]?</i></font><br>"
				if(!selfdestructing)
	...
	Topic(href, href_list)
		if(href_list["betraitor"])
			...
			switch(rand(1,2))
				if(1)
					temptext = "<font colour=red><i><b>Double-crosser. You planned to betray us from the start. Allow us to repay the favor in kind.</b></i></font>"
					src.updateUsrDialog()
