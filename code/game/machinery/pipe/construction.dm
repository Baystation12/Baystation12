...
/obj/item/pipe/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	...
	switch(pipe_type)
		if(PIPE_SIMPLE_STRAIGHT, PIPE_SIMPLE_BENT)
			...
			P.build_network()
			if (P.node1)
				P.node1.atmos_initialize()
				P.node1.build_network()
			if (P.node2)
				P.node2.atmos_initialize()
		...
		if(PIPE_MANIFOLD)		//manifold
			...
			M.build_network()
			if (M.node1)
				M.node1.atmos_initialize()
				M.node1.build_network()
			if (M.node2)
				M.node2.atmos_initialize()
				M.node2.build_network()
			if (M.node3)
				M.node3.atmos_initialize()
