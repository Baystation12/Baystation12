...
/*
		if(PIPE_SUPPLY_STRAIGHT, PIPE_SUPPLY_BENT)
			...
			P.build_network()
			if (P.nodes[1])
				P.nodes[1].initialize()
				P.nodes[1].build_network()
			if (P.nodes[2])
				P.nodes[2].initialize()
				P.nodes[2].build_network()
		...
		if(PIPE_MANIFOLD)		//manifold
			...
			M.build_network()
			if (M.nodes[1])
				M.nodes[1].initialize()
				M.nodes[1].build_network()
			if (M.nodes[2])
				M.nodes[2].initialize()
				M.nodes[2].build_network()
			if (M.nodes[3])
				M.nodes[3].initialize()
				M.nodes[3].build_network()
		...
		if(PIPE_MANIFOLD4W)		//4-way manifold
			...
			if (M.nodes[3])
				...
				M.nodes[3].build_network()
			if (M.nodes[4])
				M.nodes[4].initialize()
				M.nodes[4].build_network()
		...
		if(PIPE_MVALVE)		//manual valve
			...
			V.build_network()
			if (V.nodes[1])
//					world << "[V.nodes[1].name] is connected to valve, forcing it to update its nodes."
				V.nodes[1].initialize()
				V.nodes[1].build_network()
			if (V.nodes[2])
//					world << "[V.nodes[2].name] is connected to valve, forcing it to update its nodes."
				V.nodes[2].initialize()
				V.nodes[2].build_network()
		...
		if(PIPE_GAS_FILTER)		//gas filter
			...
			if (P.nodes[2])
				...
				P.nodes[2].build_network()
			if (P.nodes[3])
				P.nodes[3].initialize()
				P.nodes[3].build_network()
		...
		if(PIPE_MTVALVE)		//manual t-valve
			...
			if (V.nodes[2])
				...
				V.nodes[2].build_network()
			if (V.nodes[3])
				V.nodes[3].initialize()
				V.nodes[3].build_network()
