...
/obj/machinery/washing_machine/verb/start()
	...
	update_icon()
	addtimer(new Callback(src, /obj/machinery/washing_machine::wash()), 20 SECONDS)
		...
		if(isitem(A))
			...
			if(istype(A, /obj/item/clothing))
				...
				if(detergent)
					...
					addtimer(new Callback(C, /obj/item/clothing::change_smell()), detergent.smell_clean_time, TIMER_UNIQUE | TIMER_OVERRIDE)
	QDEL_NULL(detergent)
