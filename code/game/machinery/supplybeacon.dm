...
/obj/machinery/power/supply_beacon/proc/activate(var/mob/user)
	if(expended)
		return
	if(surplus() < 500)
		to_chat(if(user) user, "<span class='notice'>The connected wire doesn't have enough current.</span>")
		return
	set_light(3, 3, "#00CCAA")
	icon_state = "beacon_active"
	use_power = 1
	to_chat(if(user) user, "<span class='notice'>You activate the beacon. The supply drop will be dispatched soon.</span>")
...
/obj/machinery/power/supply_beacon/proc/deactivate(var/mob/user, var/permanent)
	...
	to_chat(if(user) user, "<span class='notice'>You deactivate the beacon.</span>")
