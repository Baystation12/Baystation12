...
/datum/antagonist/proc/greet(var/datum/mind/player)
	...
	// Basic intro text.
	to_chat(player.current, "<span class='danger'><font size=3>You are \an [role_text]!</font></span>")
...
/datum/antagonist/proc/set_antag_name(var/mob/living/player)
	...
	var/newname = sanitize(input(player, "You are \an [role_text]. Would you like to change your name to something else?", "Name change") as null|text, MAX_NAME_LEN)
	if (newname)
