...
/mob/living/proc/dpray(var/msg as text)
	...
	src.visible_message("\The [src] bows [target.current.gender == MALE ? "his" : target.current.gender == FEMALE ? "her" : "their"] head, muttering something under [target.current.gender == MALE ? "his" : target.current.gender == FEMALE ? "her" : "their"] breath.", "<span class='notice'>You send the message \"[msg]\" to your master.</span>")
