...
/datum/antagonist/godcultist/post_spawn()
	if(!GLOB.deity?.current_antagonists.len)
		return
...
/mob/living/proc/dpray(msg as text)
	...
	if(!mind || !GLOB.godcult?.is_antagonist(mind))
		return
	msg = sanitize(msg)
	...
	visible_message(
		"\The [src] bows their head down, muttering something.",
		SPAN_NOTICE("You send the message \"[msg]\" to your master.")
	)
