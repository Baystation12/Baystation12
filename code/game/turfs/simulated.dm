...
/turf/simulated/proc/wet_floor(wet_val = 1, overwrite = FALSE)
	...
	timer_id = addtimer(new Callback(src, /turf/simulated::unwet_floor()),8 SECONDS, TIMER_STOPPABLE|TIMER_UNIQUE|TIMER_NO_HASH_WAIT|TIMER_OVERRIDE)
	...
	if (istype(A,/mob/living))
		...
		if(src.wet)
			...
			if(M.slip("the [floor_type] floor", slip_stun))
				addtimer(new Callback(M, /mob::slip_handler(), M.dir, slip_dist - 1, 1), 1)
			else
				M.inertia_dir = 0
		else
			M.inertia_dir = 0
