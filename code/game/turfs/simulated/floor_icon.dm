		...
		if (has_smooth && flooring.flags & TURF_HAS_INNER_CORNERS)
			for(var/direction in GLOB.cornerdirs)
				if((has_smooth & direction) == direction)
					if(!flooring.test_link(src, get_step(src, direction), FALSE))
...
/decl/flooring/proc/test_link(var/turf/origin, var/turf/T, var/countercheck = TRUE)
	var/is_linked = FALSE
	if (!countercheck || (countercheck && test_link(T, origin, FALSE)))
