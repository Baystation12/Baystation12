/turf/simulated/floor/attackby(var/obj/item/C, var/mob/user)
	...
	if(flooring)
		if(isCrowbar(C))
			...
			if(broken || burnt)
				...
				if (flooring.remove_timer)
					...
					if (do_after(user, flooring.remove_timer, src, DO_PUBLIC_UNIQUE, do_skill = SKILL_CONSTRUCTION, delay_flags = DO_AFTER_TIME_FLAG_USER_SKILL))
						user.visible_message(
		...
		if(istype(C, /obj/item/stack))
			...
			if(use_flooring.build_time && !do_after(user, use_flooring.build_time, src, DO_PUBLIC_UNIQUE, do_skill = SKILL_CONSTRUCTION, delay_flags = DO_AFTER_TIME_FLAG_USER_SKILL))
				return
		...
		// Repairs and Deconstruction.
			...
			if(welder.isOn() && (is_plating()))
				if(broken || burnt)
					...
					if(welder.remove_fuel(0, user))
						...
						if(do_after_construct(user, src, DO_AFTER_TIME_SHORT) && welder.isOn() && welder_melt())
			...
			if(do_after(user, DO_AFTER_TIME_SHORT, src, DO_PUBLIC_UNIQUE) && welder_melt())
