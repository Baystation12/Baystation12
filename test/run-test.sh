...
function need_cmd {
	if command -v $1 >/dev/null 2>&1; then
		...
		err "program '$1' is missing, please install it"
	fi
}
...
function find_code {
	...
	cd $CODEPATH
	if [[ ! -d ./code ]]; then
		err "invalid CODEPATH: $PWD"
	else
		msg "found code at $PWD"
	fi
}
...
function run_byond_tests {
	...
	find_byond_deps
	if [[ -z "${MAP_PATH+x}" ]]; then
		exit 1
	else
		msg "configured map is '$MAP_PATH'"
	fi
	...
	run_test "check globals unchanged" "md5sum -c - <<< '8b7c58b587059f124749fc2f571642a1 *code/_helpers/global_access.dm'"
